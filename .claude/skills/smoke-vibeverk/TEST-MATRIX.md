...
- **Existing flow: `<name>`** — already implemented in
  ...
  `member-role-scope`, `editor-role-scope`).
...
| # | Test | Tenant(s) | Coverage | Pass = |
|---|---|---|---|---|
| B4.1 | Anon booking calendar correctly excludes already-taken slots | Vibeverk | Not yet done — needs extreme care given the no-production-writes-without-approval rule; not attempted this round | The taken slot does not appear as bookable; `get_taken_booking_slots()` RPC call in Network tab succeeds (200, not a caught/logged error) |
| B4.2 | Same test against Sunnvask-demo | Sunnvask-demo | **Done live 2026-07-19, at the mechanism level, not the full UI.** Sunnvask-demo has zero configured `booking-assets` (confirmed via `store` query — no public booking resource exists at all, matching Codex's own independent finding), so the visual calendar can't be exercised end-to-end without first adding a real, visible demo resource. Instead: inserted a real, `PWTEST-`-tagged row directly into `bookings`, called `get_taken_booking_slots()` via a genuine anon REST request (not SQL), confirmed the exact `{asset_id, date, time}` tuple came back with zero PII fields, separately confirmed a direct anon `SELECT` on `bookings` still returns `200 []` (RLS intact), then deleted the test row. | **PASS** at the mechanism/RLS level — the exact regression this migration was written to fix (anon calendar blind to taken slots) cannot reproduce. Full visual calendar walk-through still needs a real booking resource configured on the demo tenant first. |
...
| # | Test | Tenant(s) | Coverage | Pass = |
|---|---|---|---|---|
| B5.1 | Full visitor-widget checklist (per `vibeverk-qa` agent's own template) | Sunnvask-demo | **Done live 2026-07-19, with a real admin login (`member@vibeverk.no`) and two separate real browser tabs (anon visitor + admin).** Uncovered a genuine, previously-unknown bug in the process: admin's "Online" toggle never actually wrote the `chat-heartbeat` row to the database (a fire-and-forget `.upsert()` never dispatched, since supabase-js v2's query builder only sends its HTTP request when `.then()`/`await` is invoked) — every visitor always saw the offline fallback regardless of admin presence. Fixed and deployed same day (0.63.0), then reverified: after the fix, toggling "Online" correctly wrote the row (confirmed via direct SQL), the anon visitor's widget correctly switched to the live "Vi er tilgjengelig nå" composer, a real tagged test message sent as the visitor appeared in the admin panel within ~3s, and a real admin reply was confirmed (via direct SQL + the visitor's own `localStorage` cache) to have synced back down to the visitor side. All test rows deleted afterward, admin toggle restored to its original "Offline" state. | **PASS** (after the fix) — see `docs/project/CHANGELOG.md` 0.63.0. Live, no-reload visual re-render of the reply in the visitor widget was not cleanly observed due to this test session's own browser-automation tooling (the visitor tab stayed backgrounded, `document.visibilityState: "hidden"`, throttling its poll timer) — a testing-tool limitation, not a demonstrated product bug, since the underlying data (both messages, correct sender fields) was confirmed present in the visitor's local cache. |
...
| B5.3 | Offline form shown when admin heartbeat stale (>5 min) | Sunnvask-demo | **Incidentally confirmed live 2026-07-19** as a side effect of the B5.1 bug above — before the fix, the widget consistently showed the offline fallback ("Vi er ikke tilgjengelig akkurat nå") since `chat-heartbeat` was never written at all (a permanently-stale, not just >5-min-stale, state). Same code path (`heartbeatFresh` check), so this counts as confirmed, though not via the originally-sketched 5-minute-wait method. | **PASS** — confirmed as the pre-fix baseline behavior during B5.1 |
...
| B7.3 | CRM "Svar" reply (outbound email via `send-reply`) | Sunnvask-demo | **Done live 2026-07-19.** Created a real `PWTEST-`-tagged CRM contact, sent a real reply via the "E-post" composer. First attempt surfaced a real, previously-undetected gap: Sunnvask-demo's `send-reply` function was deployed (2026-07-19, section 0 closure) but its `RESEND_API_KEY` secret was never actually set — the send failed with "RESEND_API_KEY ikkje satt i secrets". User located and supplied a valid key from their Resend account (confirmed against the domain verified for `vibeverk.no`), set via `npx supabase secrets set` on Sunnvask-demo. Retried: send succeeded (`crm_comms` row created, `type: "email_sent"`), and the user independently confirmed real receipt in their own inbox — genuine end-to-end proof, not just a non-erroring API response. Note: `resendMessageId` came back `null` on this send — a separate, already-documented non-critical gap (the follow-up call that fetches the real RFC5322 Message-ID for future reply-thread-matching failed silently; doesn't affect delivery, only a later inbound reply's thread-matching for this specific message) — not investigated further this round. All test rows deleted afterward. | **PASS** — real outbound email sent and received, confirmed by the user directly. Root-cause gap (missing secret) fixed as a side effect. |

### B8. Workspace: roles (admin/editor/member), tasks, user administration

| # | Test | Tenant(s) | Coverage | Pass = |
|---|---|---|---|---|
| B8.1 | Invite member → member gets correct default role, admin-only UI (e.g. user-admin panel itself) hidden from non-admins | Both (currently staging only) | **Existing flow: `member-role-scope`** — implemented and verified live PASS 2026-08-05, closing the gap this row originally described (`user-deletion`/`backup-restore` invite but never log in as the member) | `[data-inav="users"]` absent from nav, direct `#/users` navigation denied, an ungated module still reachable (proves the session is genuinely functional) — already proven, re-run is regression coverage |
| B8.2 | Editor role: correct subset of permissions (can edit but not manage users, e.g.) | Both (currently staging only) | **Existing flow: `editor-role-scope`** — implemented and verified live PASS 2026-08-05, same technique as B8.1 applied to `role='editor'`. **Caveat**: as of this writing only ONE module (`workspace/module-users.js`) is role-gated at all, so this flow's assertions are currently IDENTICAL to `member-role-scope`'s — real coverage for the one gate that exists, not proof editor/member differ in the product today. This row's original "editor-appropriate UI is visible" scope (a genuinely DIFFERENT permission surface from member) is not yet implemented in the product, so there's nothing broader for a flow to assert yet | Same as B8.1 — already proven for the gate that exists, re-run is regression coverage |
...
| # | Test | Coverage | Pass = |
|---|---|---|---|
| D.1 | No leftover test/dev data visible anywhere a prospective customer would see during a live demo (CRM customer list, bookings calendar, chat history, references section) | **Done live 2026-07-19** — the two `PWTEST-` leads/CRM customers left over from this session's own B2/B3.2 testing were found (flagged by Codex's independent review) and deleted; a full sweep afterward for `PWTEST%`/`SMOKETEST%` across `leads`, `crm_customers`, `bookings`, `chat_conversations`, `chat_messages`, `tasks`, and `notes` came back **zero** everywhere | **PASS** — confirmed via direct SQL, not just a visual walkthrough |
