...
- **`member-role-scope`/`editor-role-scope` flows only**: `VW_STAGING_SERVICE_ROLE_KEY` (Dashboard → Settings → API → "service_role" secret — a DIFFERENT, more sensitive credential than the anon key above, since it bypasses RLS). Used ONLY server-side via Node's own `fetch()` to call the Admin API's `generate_link`, never exposed to a browser page. **`vibeverk-staging`'s Auth → URL Configuration → Redirect URLs must include `http://localhost:8080/**`** — confirmed live 2026-08-05 that without this, `generate_link`'s `redirect_to` is silently ignored (falls back to the project's Site URL, a real deployed Vercel URL, not this local static server) with no error surfaced. Also confirmed live: `redirect_to` is a TOP-LEVEL field on the raw REST endpoint, NOT nested under `options` (that nesting is a supabase-js SDK-only convenience for its client-side wrappers) — nesting it here is also silently ignored, same failure mode, different cause.
...
| `member-role-scope` / `editor-role-scope` | **implemented and verified live PASS 2026-08-05** against `vibeverk-staging` | Closes a real gap: `user-deletion`/`backup-restore` both invite a throwaway member via the real UI but never log in AS them, so role-gated UI visibility was never actually exercised. Uses the Admin API's `generate_link` (magiclink) to get a real, working session as an invited `member`/`editor` in a SEPARATE browser context (never the admin's own, since that would just reuse the admin's already-authenticated session on the same origin) — no password, no real email. Asserts: `[data-inav="users"]` (admin-only `workspace/module-users.js`, `roles:["admin"]`) is absent from that user's nav, direct navigation to `#/users` shows the "ikkje tilgang"-denial text rather than the real list, and an ungated module (`dashboard`) is still reachable (proving the session is genuinely functional, not just broadly broken). As of this writing only ONE module in the whole app is role-gated at all, so `member` and `editor` currently produce IDENTICAL assertions — real regression coverage for the one gate that exists, not proof the two roles differ today (TEST-MATRIX.md's B8.2 "editor-appropriate UI" scope is broader than what's actually implemented). Cleans up the throwaway user via the real UI, same orphan-safety-net pattern as `user-deletion`. |
