...
**Console «Læring» oppdatert: reell tabell-layoutbug retta, og ny arkitektur-illustrasjon lagt til.**
...
- Ingen database-, Supabase- eller produksjonsendring. Alle tre testsuitene grøne (778/303/132, 0 FEIL). Cache-bust: `console-core.js` 286→287.

## 0.160.0 — 2026-08-24

**W3C Trace Context (`traceparent`) request-sporing lagt inn som grunnleggende plattform-infrastruktur**, utløst av at Supabase nå støtter denne standarden direkte i sine egne loggar. Vibeverk hadde tidlegare ingen request-/trace-ID noko stad — ei full revisjon fann null korrelasjons-ID mellom klient, Vercel Functions og Supabase. Sjå `docs/architecture/tracing.md` for full grunngjeving og kjede.

- Ny delt helper `api/_lib/trace.js` (`generateTraceparent()`/`getOrCreateTraceparent()`), brukt av `middleware.js` og alle `api/*.js`-funksjonar som kallar Supabase (`tenant-config.js`, `qr-redirect.js`, `_lib/resolve-tenant.js`, `_lib/tenant-manifest.js` og dei tre manifest-funksjonane, `ai/annual-wheel.js`, `ai/oversikt.js`).
- `middleware.js` genererer/gjenbruker traceparent éin gong per request, sender han vidare på sine eigne to `fetch()`-kall til kontrollplanet, og forwardar han som request-header til alle `rewrite()`/`next()`-mål (les: alle `api/*.js`-funksjonane) samt som synleg responsheader.
- Klient-side: `core.js` genererer sin eigen traceparent ved sideinnlasting (kan ikkje lese `/config.js` sin responsheader sidan han vert lasta via `<script>`-tag, ikkje `fetch()`) og sender han med på `supabase-js`-klienten (`App.supabase`). `console/console-core.js` sine to eigne `createClient()`-kall og `workspace/workspace-core.js` gjenbruker same mønster/id.
- Bevisst utanfor omfang denne runda: ingen logg-viewer/dashboard, ingen Sentry/OpenTelemetry, ingen endring i `supabase/functions/*` eller sjølve `supabase-control/`s `broker`-funksjon (berre kallsida i Console sender headeren no).
- Ingen lagring noko stad (ikkje cookie/localStorage/DB) — reint in-flight header-plumbing, verifisert å ikkje utløyse cookie-samtykke eller ny personopplysningskategori.
- 9 nye testar i `test-api.js` (traceparent-format, gjenbruk av gyldig innkomande, forkasting av misforma innkomande, forwarding på `resolveTenantByHostname()` og gjennom `middleware.js`). Full testkøyring grøn: `test.js` 778/778, `test-workspace.js` 303/303, `test-api.js` 132/132, 0 FEIL i alle tre. Cache-bust: `core.js` 96→97, `workspace-core.js` 32→33, `console-core.js` 285→286.
