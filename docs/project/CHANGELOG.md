...
**Ny "Strategi"-seksjon i Console + vesentleg utvida "Læring".** Frode: *"Ønsker at strategi, roadmap etc. er tilgjengelig i console"* + *"Vesentlig forbedre og endre 'Læring' i console. Vi har gjort svært mykje sidan vi oppretta den."*

- **Ny "Strategi"-seksjon** (`internt`-gruppa, kompass-ikon): viser `STRATEGY.md`, `ROADMAP.md`, `CURRENT_STATE.md` og alle 15 ADR-ar (`docs/decisions/`), gruppert i sidemeny (Strategi / Roadmap / Status / Avgjerder). Halde adskilt frå Læring per eksplisitt brukarval (to seksjonar, ikkje éi samla "Dokumentasjon") -- ulikt formål (styring/avgjerder vs. opplæring).
- **"Læring" utvida frå 4 til 13 kurerte dokument**, gruppert (Kom i gang / Trygge endringar / Tryggleik / Kundeleveranse) i staden for éin flat rad med faner -- flate faner skalerte ikkje forbi 4-5 dokument, og begge seksjonane har no langt fleire.
- **Kuratert utval, ikkje heile `docs/`-treet** (eksplisitt brukarval): `docs/archive/`, `docs/marketing/` og interne compliance-arbeidsnotat er medvite utelatne.
- **Felles, ny sidemeny+innhald-visingsmekanisme** (`renderDocSection()`) erstattar den gamle flate fane-rada -- delt av begge seksjonane, ikkje duplisert kode.
- **Interne md-til-md-lenker fungerer no inni Console**: ei lenke i eit dokument (t.d. ROADMAP.md sin lenke til CURRENT_STATE.md) som peikar på eit anna kuratert dokument (i BEGGE seksjonane) vert fanga opp og navigerer inni Console, i staden for berre å opne rå Markdown-tekst. Ei lenke som ikkje er i det kurerte utvalet får i staden `target="_blank"` (opnar rå Markdown i nytt vindauge, forlèt ikkje Console-fana). Verifisert direkte at denne konvensjonen faktisk varierer på tvers av docs/ -- STRATEGY.md viser til andre filer som vanleg kode-tekst (ikkje ekte lenker), medan ROADMAP.md/decisions/README.md bruker ekte markdown-lenker -- interceptet fangar berre opp det siste mønsteret, som forventa.
- **Retta ein reell, føre-eksisterande inkonsistens** oppdaga undervegs: kommentaren over `NAV_ITEMS` hevda at "laring" (og no "strategi") aldri les den valde kunden, men berre `arctic`/`kundeanalyse`/`compliance` hoppa faktisk over å vente på `getSC()` i `renderSection()`-dispatchen. Læring venta difor unødig på ei tenant-lasting han aldri brukte. Retta for begge (Strategi + Læring).
- **AI Lab sitt "Læringsutkast"-publiseringsløp** (kobling frå eit Gemma-generert utkast til eit faktisk synleg dokument i Læring) er medvite HALDE UTANOM denne runda -- brukaren er framleis i test-fasen med den funksjonen og ønskte ikkje å byggje vidare på han enno.
- Eksisterande Console-testsuite (`test-ai-lab-console.js`) oppdatert til den nye standard-dokument/gruppe-strukturen (venta fyrste-doc-innhald/label endra). Alle andre Console-testsuiter (`test-arctic-console.js`, `test-compliance-console.js`, `test-kundar-console.js`, `test-privacy-console.js`) framleis grøne, uendra. Ny funksjonalitet verifisert manuelt via ein throwaway jsdom-harnes (same mønster som dei eksisterande testsuitene) -- seksjonane opnar, sidemenyen grupperer korrekt, og lenke-interceptet fangar opp/navigerer korrekt -- før scriptet vart sletta att.
- **UX/Mobile Reviewer-funn retta før merge**: (1) den sticky sidemenyen hadde ingen eigen scroll -- ein lang gruppe (Avgjerder-gruppa åleine har 16 rader) kunne verte høgare enn viewporten og gjere dei nedste radene vanskelege å nå medan menyen var sticky-fest; retta med `max-height`/`overflow-y:auto` på sidemenyen sjølv. (2) På det smale (≤860px) stabla viewporten var det berre gruppe-*label*-teksten som vart tvinga på eiga rad, ikkje sjølve gruppe-elementet -- to korte grupper kunne difor ende opp side om side i staden for tydeleg stabla; retta med `width:100%` på sjølve gruppe-elementet. (3) `.cs-doc-nav__item` mangla ein eigen `:focus-visible`-ring, inkonsekvent med andre nyare interaktive element i same fil; lagt til. (4) Den aktive dokument-knappen hadde berre eit visuelt `.is-active`-utslag, ingen ikkje-visuelt signal for skjermlesar-brukarar; `aria-current="page"` lagt til.
...
Frode: *"Kan du ta det som er på enkelt/middels?"* -- same oppfølging som 0.167.0, no fullført.

- **Mobilvisning**: ein ny "▭ Mobilvisning"-knapp i live-edit-verktøylinja opnar ein modal med sida sjølv lasta inn att i ein telefonramma sandboksa iframe (375px viewport-breidde) -- same filosofi som Console sin `pbc-preview-iframe` (`pbRenderPreviewInto()`): operatøren skal sjå EKTE rendering, ikkje ein tilnærma kopi bygd på nytt her. I motsetnad til Console sin side-byggjar har den offentlege sida ikkje eit isolert seksjons-array å rendre frå -- heile appen bootstrapar seg sjølv frå Store/localStorage kvar gong, så løysinga går via ein ekte iframe-reload (cache-buster-query-parameter) i staden for ein eigen render-funksjon. `saveContent()`/`saveNavSettings()` skriv alltid SYNKRONT til localStorage, så reload-en viser oppdatert innhald med det same, heilt uavhengig av den debounca Supabase-synken -- verifisert direkte (rediger ein tekst medan modalen er open, iframe-src endrar seg automatisk).
- **CSP-endring naudsynt**: `frame-src` i `index.html` sin Content-Security-Policy tillet berre `https://plausible.io` frå før -- utan `'self'` lagt til ville nettlesaren blokkert denne heilt nye same-opphav-iframen. `sandbox="allow-scripts allow-same-origin"` (må ha `allow-scripts` sidan iframe-en bootstrapar heile appen sin eigen JS, ulikt Console sin reint statiske HTML+CSS-førehandsvising).
- **Reell produksjonsbug oppdaga og retta, uavhengig av mobilvisings-funksjonen sjølv**: Playwright-verifisering av den nye iframen synte at "Bytt bilete"/"Kopier"-knappane (`C.liveEditImageBtn()`/`C.liveEditDupBtn()`, rendra ubetinga av malane kvar gong hero/about/eit tenestekort har eit bilete) var SYNLEGE for KVAR vanleg besøkande på den ekte offentlege sida, ikkje berre inni live-edit. Rotårsak: styletaggen som skjuler dei via `display:none` (`liveEditStyleTag()`) vart FØR berre injisert frå `setLiveEditMode()`, altså aldri for ein besøkande som ikkje sjølv opnar admin/live-edit. Stadfesta direkte mot ei reint offentleg, urørt sidevising (ingen admin i det heile): knappen synte `display:block`, plassert midt oppå framsidebiletet. Fiksa ved å kalle `liveEditStyleTag()` uvilkårleg ved vanleg oppstart (`boot()` i `actualInit()`), ikkje berre når nokon faktisk startar live-redigering -- funksjonen er idempotent (guarda via `document.getElementById`), så dette er trygt uavhengig av om live-edit nokon gong vert brukt.
- **Security Auditor-funn retta før merge** (ingen BLOCKER/HIGH): (1) `frame-src 'self'`-kommentaren i `index.html` overdreiv kva han faktisk vernar mot -- retta til å presisere at `frame-src` berre styrer kva DENNE sida har lov å ramme inn, ikkje motsett (det krev `frame-ancestors`/`X-Frame-Options`, som ikkje finst nokon stad for det offentlege nettstaden i dag -- eit stadfesta, føreeksisterande gap, ikkje noko denne endringa gjer verre eller betre). (2) Kvar iframe-reload av mobilvisinga (ved kvar lagring medan modalen er open) vart tidlegare talt som ein EKTE ny sidevisning av Plausible/intern sidetelling -- retta ved at begge no hoppar over sporing når URL-en inneheld iframe-en sin eigen `_mp=`-cache-buster-parameter (`initAnalytics()` i core.js, modulporten i module-sidetelling.js).
- **UX/Mobile Reviewer-funn retta før merge** (to HIGH): (1) den flytande live-edit-verktøylinja og «Avslutt redigering»-knappen (z-index 9999) synte seg OPPÅ mobilvisings-/skrift-modalen (z-index 100) i ekte nettlesar-testing -- dekte hint-teksten heilt, og i liggjande mobilformat delar av sjølve telefonramma. Fiksa ved at BEGGE modalane no legg ein `vc-live-edit-modal-open`-klasse på `<body>` som skjuler dei to elementa heilt medan modalen er open (fjernast att ved lukking). (2) `.vc-mp-frame` sin opphavlege `max-width:100%` let ramma KRYMPE ned mot ca. 299px inni ein smal admin-modal på ein 375px-brei skjerm -- nettopp den breidda ein admin sjølv på mobil ville brukt funksjonen frå fekk difor eit falskt, smalare mobilbilete enn sida sine eigne brytepunkt faktisk målrettar. Fiksa med ein eigen skrollande ytre-behaldar (`.vc-mp-scroll`) slik at ramma alltid held ekte 375px, stadfesta direkte (`getBoundingClientRect()`: 355px iframe-innhald + 20px kant = 375px totalt, uendra ved smalare modal).
- Verifisert i ekte nettlesar mot ekte produksjonsinnhald (Playwright, ingen skriving -- stadfesta via reload): mobilvisings-knapp/modal/iframe fungerer, ingen CSP-brot i konsollen, den EKTE offentlege sida (frisk, urørt) syner no korrekt INGEN admin-overlegg-knappar, verktøylinje-kollisjonen og feil iframe-breidde stadfesta retta (portrett 375×667 og liggjande 667×375). `test.js` utvida til 942 OK / 0 FEIL.
...
**Kontraktsavgrensing oppdaga og respektert**: gjekk gjennom `core.js` sine faktiske `registerModule()`-kall (ikkje berre dokumentasjon) og stadfesta at KUN `hjem`/`om-oss`/`tjenester` er malstyrte via `resolveTemplate()` — `aktuelt` kallar `C.news()` direkte, og Referansar (`module-references.js`) og Quiz (`module-quiz.js`) er eigne, delte modular med sin eigen faste stil. Malen implementerer difor berre `hero()`/`about()`/`services()`; Aktuelt/Referansar/Quiz vil vise seg med plattforma sin vanlege stil uansett kva mal som er aktiv, og er eksplisitt UTANFOR omfanget av denne endringa.

**Innhaldsmapping** (avklart med brukaren, som sjølv korrigerte ei feilantaking undervegs — "Bak Vibeverk" i mockupen ER Om-seksjonen, ikkje eit eige, manglande felt): Hero → `content.hero`, "Bak Vibeverk" (mørk sitat-/portrettseksjon) → `content.about` (`heading`/`intro`/`text`/`image`, ingen ny platform-felt trengst), Tenester → interaktiv fane-veljar bygd generisk frå `content.services.cards` (vilkårleg lengde, ikkje hardkoda til 3). Ein tidlegare vurdert eigen "Om Vibeverk / tre grunnar"-seksjon vart droppa — han hadde ingen heim i den faktiske innhaldsmodellen og var i praksis ei duplisering av Om-seksjonen i ei inkompatibel form.

**Reint dekorativt, hardkoda i sjølve malen** (ikkje kunde-/admin-redigerbart, sidan dette er ein einTenant-bespoke mal): terminal-typeanimasjonen i heroet (Vibeverk-flavored `vibeverk deploy`/`status`/`ai`/`sikkerheit`-sekvensar, roterer i loop, respekterer `prefers-reduced-motion`), samt det animerte rutenett+sveip-mønsteret i "Bak Vibeverk"-bakgrunnen. Søkje-kommandopaletten og dei elaborate scroll-pinna kinoscenene frå mockupen vart bevisst IKKJE tekne med i denne fyrste runden — halde nede for å gjere endringa overkomeleg å gjennomgå.

**Testdekning**: `template-vibeverk-cinema.js` lagt til `index.html` (cache-bust `?v=1`, etter `template-vedvik-test.js`) og `test.js` sitt delte skript-eval-lasteliste, pluss 4 nye assertions som stadfestar at `window.SiteTemplates["vibeverk-cinema"]` finst og eksponerer `hero`/`about`/`services` som funksjonar. Full suite: 783/303/124 OK, 0 FEIL.

**Ikkje gjort, krev eksplisitt godkjenning per CLAUDE.md sitt deploy-vern**: å faktisk TILDELE/aktivere denne malen på den live Vibeverk AS-tenanten via Console. Filen ligg klar i repoet, men er ikkje aktiv nokon stad enno.

## 0.160.0 — 2026-09-08

**Ny modul: `module-quiz.js` — interaktiv "Kjapp sjekk"-quiz på forsida.** Vaks ut av eit bespoke design-mockup-arbeid (`vibeverk-template`, eit separat side-prosjekt for Vibeverk AS sin eigen framtidige nettside-mal — sjå den mockupen sin eigen historikk for scroll-story/synthwave-eksperimenta). Brukaren opplevde quiz-mekanikken der som genuint gjenbrukbar for andre kundar, ikkje berre eit eingongstriks, og bad om ein arkitektur-vurdering (Arkitekt-agenten) før bygging, per CLAUDE.md sin standard "medium/stor funksjon"-terskel.

**Modellen**: N spørsmål (admin legg sjølv til/fjernar), kvart med M svaralternativ, kvart alternativ med ein tallverdi. Totalscore = summen av valde verdiar. Resultatet vert vist ut frå kva "nivå" (min/max-poengintervall, òg fritt admin-redigerbart i tal) totalen hamnar i — erstattar mockupen sin hardkoda "tel nei-svar"-logikk med noko som faktisk generaliserer til andre bransjar/spørsmål.

**Bevisst avgrensing, eksplisitt kravd av brukaren**: modulen samlar ALDRI inn eller lagrar noko om kva ein besøkjande faktisk svarte — poengsummen vert rekna ut og gløymt i nettlesarminnet, aldri sendt nokon stad, aldri i localStorage. Ingen `App.addLead()`-kall, ingen samtykkeboks. Stadfesta (ikkje anteke) ved å faktisk spore samtykke-/personvern-mekanismen i koden (`App.addLead()`/`buildConsentSnapshot()`/`computeDefaultPrivacyText()`) og verifisere at quiz-en aldri rører nokon av dei — Privacy/Compliance Advisor-porten i CLAUDE.md gjeld difor ikkje for denne modulen, GITT at implementasjonen held seg unna svarlagring for alltid (dokumentert som ein varig føresetnad i modulfila sjølv, ikkje ein eingongssjekk).

**Plassering**: inline på forsida (`order: 15`, mellom Hjem og Om oss), ikkje ei skjult eiga side slik FAQ er — brukaren ønskte han synleg tidleg, som ein konverteringsstartar rett under heltebiletet.

**Feature-flagg** `features.quiz` — MÅ vere eksplisitt `false` som standard (nytt/uprøvd, same konvensjon som sidebygger/sidetelling/qrCode), lagt til i Console → Modular (`FEAT_LABELS`/`OPT_IN_FEATURES`/`FEAT_HELP`).

**Testdekning**: `test.js` fekk `module-quiz.js` inn i skript-lastelista (syntakssjekk) og ein assertion på at modulen ikkje viser seg sjølv når flagget er av. Full på/av-CRUD-flyt vart verifisert manuelt via eit eige jsdom-skript (score-utrekning, tier-treff, eiga CTA-overstyring, restart, ingen svar-persistering) sidan denne test-riggen sin eingongs-`App.ready`-port gjer det upraktisk å teste "flagg PÅ" inline i den delte testlasten utan å endre sjølve rigg-strukturen — same avgrensing gjeld alt for `module-qrcode.js`, som heller ikkje er dekt der. Admin-CRUD-koden attbruker FAQ sitt alt-testa liste/redigerings-mønster ord for ord, så eigen dekning der vart vurdert som lågare risiko.

## 0.159.21 — 2026-09-07

**Ukentleg teknisk kontroll (Supabase-logging / AI-arkitektur / agent-sikkerheit / hosting-portabilitet) — ingen kodeendring naudsynt, éin dokumentasjonsoppdatering.** Fire-punkts revisjon bestilt som ein periodisk sjekk, ikkje eit oppdaga problem:

1. **Supabase Management API sitt `logs.all`-endepunkt**: ikkje i bruk noko stad i Vibeverk, Console, helse-/overvakingskode eller `scripts/` — den einaste Management API-bruken som finst er prosjekt-*konfigurasjon* (SMTP/OAuth-oppsett, nøkkel-henting i tenant-onboarding), urelatert til logg-spørring. Ikkje råka, ingen migrering naudsynt.
2. **AI-arkitektur**: dei tre produksjonsflatene som faktisk kallar ein språkmodell (`api/_lib/annual-wheel-ai.js`, `oversikt-ai.js`, `customer-analysis-ai.js`) kallar kvar for seg `https://api.anthropic.com/v1/messages` direkte, med eigen duplisert modell-konstant/env-variabel — ikkje eit felles "AI Service"/"modellprofil"-lag slik prinsippet `Vibeverk-funksjon → AI Service → modellprofil → provider/model` føreset. Fungerer stabilt i dag og INGEN Vercel AI Gateway-avhengigheit finst (sjekka eksplisitt) — vurdert som 🟡, verdt å konsolidere neste gong ein 4. AI-funksjon eller fleire-provider-støtte faktisk trengst, ikkje refaktorert no per oppdraget sitt eige "ikkje uten reelt behov"-prinsipp.
3. **Agent-sikkerheit**: alt dekt av `docs/decisions/ADR-0015-agent-tool-access-policy.md` (godkjend 2026-09-01, seks dagar før denne kontrollen) — stadfesta framleis gyldig. Eitt hol lukka: ADR-0015 sin bevis-seksjon lista opphavleg berre Smart Årshjul/Oversikt som produksjons-AI-flater; Kundeanalyse (`api/customer-analysis.js`, frå 0.128.0) var ikkje nemnd. Stikkprøve stadfesta at han uavhengig oppfyller same prinsipp (operatør-autentisering, audit-før-mutasjon, `service_role` sine `UPDATE`/`DELETE` eksplisitt revoka på dei aktuelle tabellane) — lagt til som punkt 4 i ADR-en sin bevis-seksjon, ingen åtferdsendring.
4. **Hosting-portabilitet**: kjerne-forretningslogikken (alle `api/*.js`) brukar den portable Web-standard `{ fetch: handler }`/`Request`/`Response`-forma (køyrer uendra på Vercel Edge, Vercel Node, Cloudflare Workers, Deno) — ingen `@vercel/kv`, `@vercel/blob`, `@vercel/edge-config` eller cron-avhengigheit noko stad. Den einaste Vercel-spesifikke koplinga er `next`/`rewrite` frå `@vercel/functions` i `middleware.js` sjølv (den alt dokumenterte, isolerte ruting-mekanismen for per-tenant hostname-oppløysing) — ikkje i sjølve forretningslogikken. Ingen endring naudsynt, målet (kunne byte hosting for enkelte tenester utan omfattande omskriving) er alt oppfylt for det som tel mest.

**Konkret endring**: berre punkt 4 i ADR-0015 sin bevis-seksjon (Kundeanalyse lagt til). Ingen applikasjonskode, migrering eller konfig endra.
...
- **To nye, avgrensa broker-handlingar** (`get_design_template`/`set_design_template`, `broker/index.ts`): les/skriv KUN `content.designTemplate`-feltet, aldri resten av `content`-objektet. Medvite IKKJE gjort via å leggje "content" til `set_config` sin `ALLOWED_CONFIG_KEYS` — den handlinga gjer ei FULL erstatning av heile verdien for ein nøkkel, og sidan `content` vert redigert av KUNDEN sjølv mykje oftare enn `superconfig` (som alt har ein tilsvarande, berre-åtvara-om kollisjonsrisiko andre stader i Console), ville det gjort ein operatør sin eldre nettlesar-kopi av HEILE innhaldet i stand til å overskrive ekte, nyleg kundetekst. `set_design_template` gjer i staden ei ekte les-endre-skriv heilt server-side.
- **Security Auditor-pass FØR utrulling fann eitt reelt MEDIUM-funn, retta same runde**: koden sin eigen kommentar hevda at ein feil/ukjend mal-ID "alltid fell trygt attende til klassisk" via `core.js` sin `resolveTemplate()` — men eit vanleg `reg[id]`-objektoppslag ARVAR frå `Object.prototype`. Ein `design_template`-verdi som `"constructor"`, `"toString"` eller `"__proto__"` ville difor returnert eit ARVA, TRUTHY objekt/funksjon i staden for å falle gjennom til klassisk, og teke ned rendring av HEILE den offentlege sida for tenanten. Fyrste rettingsforsøk (eit kvitlista teiknformat i `set_design_template`, kun bokstavar/tal/bindestrek) synte seg VERIFISERT UTILSTREKKELEG i etterkant — ord som "constructor"/"toString" er reine bokstavar og ville framleis sloppe gjennom. Den faktiske, robuste fiksen vart gjort i `resolveTemplate()` sjølv (`core.js`): eit eksplisitt `Object.prototype.hasOwnProperty.call(reg, id)`-sjekk før oppslaget, som kategorisk utelukkar HEILE denne problemklassen (alle arva `Object.prototype`-eigenskapar), uavhengig av kva veg ein uventa verdi kom inn frå — ikkje berre dei namna nokon tilfeldigvis tenkte på. Verifisert isolert med alle kjende farlege namn (`constructor`, `toString`, `valueOf`, `__proto__`, `hasOwnProperty`) — alle fell no korrekt attende til klassisk.
- **Console-UI**: nytt, sjølvstendig felt "Design-mal (avansert)" i Console sin eksisterande "Web"-seksjon (`renderWeb()`) — bevisst EIGEN, uavhengig `<form>`, ikkje ein del av den store farge-/font-lagringsforma, for å unngå at nokon trur dette handlar om DEN forma sin kjende kollisjonsrisiko (det gjer det ikkje, sidan denne skriv ei anna, smalare handling).
- Utrulla direkte til `vibeverk-control` sin `broker`-funksjon. `test.js` (778/778) og `test-workspace.js` (303/303) grøne. Cache-bust: `core.js` 98→99 (alle fire HTML-skal), `console-core.js` 301→302.
...
- **`components.js`**: `pbHero(d)` les no eit `height`-felt (`"auto"` (uendra standard) | `"tall"` | `"full"`) og legg til ein `pb-hero--h-*`-klasse. `pageSection(s)` les eit hero-spesifikt `data.fullBleed`-flagg og legg til `pb-sect--bleed` på den ytre `<section>`-en. Begge fell trygt attende til uendra åtferd når felta manglar (eksisterande, alt-lagra hero-seksjonar utan desse felta rendrar byte-for-byte likt som før — verifisert direkte, ikkje anteke).
- **Arkitektval** (Arkitekt-runde før koding): `fullBleed`/`height` vart lagt som hero-EIGNE datafelt, IKKJE som ein tredje verdi på den delte `variant.width`-enumen — sidan `width` blir tilbydd identisk for KVAR seksjonstype (tekst, rutenett, sitat …), og full breidde berre gjev meining for biletberande typar. Unngår ein regresjonsflate på tvers av alle eksisterande seksjonstypar.
- **CSS**: fire nye reglar (`.pb-sect.pb-sect--bleed`, `.pb-hero--h-tall`, `.pb-hero--h-full`, samt flex-sentrering for dei to sistnemnde) lagt til BÅDE i `module-page-builder.js` (den ekte offentlege sida) og `console-core.js` sin `pbPreviewCss()` (Console sin separat vedlikehaldne førehandsvisings-CSS-kopi) — same, alt-eksisterande dupliseringsmønster som resten av page-builder-CSS-en. `min-height` (aldri `height`) og `dvh` (aldri `vh`) med vilje, for å aldri klippe innhald og handtere mobil-adressefelt-kollaps korrekt (same eining alt i bruk i `module-workspaceship.js`).
- **Console-skjema**: ny «Høyde»-nedtrekksliste og «Fullbredde»-avkryssingsboks på hero-redigeringsskjemaet, med forklarande hint-tekst. Fann og retta undervegs: avkryssingsboksen vart IKKJE fanga av det generiske felt-endrings-lyttarutvalet (som berre matchar select/text/url/textarea, ikkje checkbox) — utan eigen lyttar ville han sett ut til å fungere (visuelt haka av) men aldri faktisk lagra.
- **UX/Mobile Reviewer-pass FØR ferdigstilling** (standard rutine for meiningsfulle UI-endringar) fann to HIGH-funn, begge retta same runde: (1) «Bredde»/«Luft»-nedtrekkslistene synte seg framleis fullt verksame sjølv når Fullbredde-avkryssinga i realiteten gjorde dei irrelevante — retta ved å DEAKTIVERE (ikkje skjule) dei to felta når Fullbredde er av, både ved fyrste opning og i sanntid ved avkryssing (`pbSelectField()` fekk ein ny, bakoverkompatibel `disabled`-parameter — alle 15 andre kallstadar uendra); (2) Console sin førehandsvisings-iframe kan ALDRI simulere ei ekte mobileining sin skjermhøgde eller adressefelt-kollaps nøyaktig, sidan `dvh`-einingar der løyser seg mot iframen sin eigen boks, ikkje eit ekte visningsvindauge — i staden for å late som førehandsvisinga er eit presist mål, seier mobil-etiketten no rett ut at høgda berre er rettleiande. To MEDIUM-funn (vertikal sentrering på korte liggjande skjermar, heilt tom full-høgd-hero) krev berre manuell nettlesarverifisering, ingen kodeendring.
- Ingen database- eller produksjonsendring. `test.js` (778/778) og `test-workspace.js` (303/303) grøne. Isolert jsdom-verifisering av `pageSection()`/`pbHero()`-output stadfesta både uendra åtferd for gamal data og korrekt klassepåføring for ny. Cache-bust: `components.js` (`index.html`/`admin/index.html`/`workspace/index.html`/`console/index.html`, kvar sin eigen tidlegare versjon +1), `module-page-builder.js` 5→6, `console-core.js` 295→296.

## 0.159.12 — 2026-09-03

**To ting: Fase 2 av 3 av kontrollplan-varslinga (sanntids e-postvarsel på `generate_support_access`), og ein reell Sidebygger-bugfiks (biletopplasting feila for bilete med gjennomsikt).**

- **Fase 2 (varsling)**: ny `alert-support-access`-Edge Function + ein Postgres-trigger på `broker_audit_log` (`alert_on_support_access()`, fyrer berre på den eine pending→success-overgangen for `action='generate_support_access'`) sender no Frode ei e-post med tenant/operatør/mål-brukar/tidspunkt — aldri sjølve tilgangslenka, som framleis aldri vert persistert nokon stad. Same, alt-verifiserte `net.http_post`+Vault-akkreditiv-mønster som `trigger_retention_sweep()`. Gjenbruker eksisterande `TENANT_SMTP_RESEND_API_KEY`/`TENANT_SMTP_SENDER_EMAIL`-løyndomar (Vibeverk sin eigen delte Resend-konto, alt brukt til å setje opp kundane sin eigen SMTP), ny `SECURITY_ALERT_EMAIL`-løyndom sett til `frode@hammerseth.com` (mellombels — kan endrast seinare). Security Auditor-pass FØR utrulling fann null BLOCKER/HIGH/MEDIUM, berre to LOW/kosmetiske notat (skjør strengscraping av mål-e-post frå `detail`-feltet dersom meldingsteksten nokon gong vert omskriven; ein `search_path`-stil-inkonsekvens utan reell tryggingskonsekvens) — ingen av dei kravde retting før utrulling. Fase 3 (mønster-basert varsel på `get_tenant_service_role_key`) står framleis medvite utsett til Fase 1 sin logg har samla nok reell trafikk å kalibrere terskelen mot.
- **Sidebygger-bugfiks**: biletopplasting via Console sin Sidebygger (`upload_section_image`/`upload_logo`, `broker/index.ts`) feila med «Biletet er for stort og kunne ikkje komprimerast nok automatisk» for heilt vanlege foto under 2MB, stadfesta direkte mot `broker_audit_log` (detail: «kunne ikkje komprimerast under 600KB»). Rotårsak: den 2026-08-12 fiksen løyste dette for PNG-ar UTAN faktisk brukt gjennomsikt (fell til JPEG-komprimering), men PNG-ar MED (kanskje utilsikta) ekte gjennomsiktige pikslar held fram i reint tapsfritt sporet, som berre skalerte ned til 40% før han gav opp — ofte ikkje nok for eit detaljert foto. Retta ved å utvide skaleringsstigen med to mindre steg (25%, 15%) — trygt for JPEG-sporet (som så godt som alltid lykkast lenge før 40% uansett), gir reelt meir rom for det tapsfrie PNG-sporet.
- **Utrulla og delvis verifisert direkte mot `vibeverk-control`**: stadfesta trigger og funksjon faktisk finst og trigger er aktivert (`pg_trigger`/`pg_proc`), begge Edge Functions deploya (`alert-support-access` ny, `broker` redeploya med kompresjonsfiksen). Ein fullstendig ende-til-ende-test (eit ekte testkall som ville trigga sjølve e-postutsendinga) vart blokkert av sandkassa sin skriveklassifikator for direkte databaseinnsetjing, og eg har ikkje den rå `SUPABASE_SERVICE_ROLE_KEY`-verdien for eit direkte Edge Function-kall heller — verifisert alt som er råd utan desse, resten stadfestast naturleg neste gong nokon faktisk genererer support-tilgang i Console.
- Ingen endring i noka kunde sitt eige dataplan-prosjekt utover Sidebygger-kompresjonsfiksen (som køyrer på kunden sin eigen `media`-bøtte, ingen skjemaendring). `test.js`/`test-workspace.js` upåverka (ingen fil i hovudrepoet endra utover changelog/versjon).

## 0.159.11 — 2026-09-03

**Fase 1 av 3 (logging, INGEN varsling enno) i eit nytt sporingslag for `get_tenant_service_role_key()` — kontrollplanet sin einaste funksjon som dekrypterer ei kunde sin fulle `service_role`-nøkkel frå Vault.** Følgjer direkte opp gårsdagens Security Auditor-katch-up (0.159.10) og ein etterfølgjande manuell arkitektur-gjennomgang med brukar: funksjonen logga historisk INGENTING sjølv — all eksisterande sporing (`broker_audit_log`) skjedde i KALLAR-koden (`broker`/`tenant-admin` Edge Functions). Eit rått RPC-kall med ein lekka `VIBEVERK_CONTROL_SERVICE_ROLE_KEY`, heilt utanom Edge Functions-koden, ville difor vore usporbart.

- **`supabase-control/supabase/migrations/20260903120000_key_decrypt_log.sql`**: ny, låst-ned `key_decrypt_log`-tabell (RLS aktivert, `REVOKE ALL FROM PUBLIC, anon, authenticated`, eksplisitt `GRANT SELECT ... TO service_role`, ingen policy for nokon annan rolle). `get_tenant_service_role_key()` skriven om frå `LANGUAGE sql` til `LANGUAGE plpgsql`, same signatur/`SECURITY DEFINER`/`search_path`, no med ein `INSERT` FØR sjølve dekrypterings-`SELECT`-en — fail-closed med vilje (feilar loggings-INSERT-en, feilar heile kallet, ingen nøkkel vert nokon gong returnert utan at han fyrst er logga). Dagleg opprydding (`pg_cron`, rader eldre enn 90 dagar) for å hindre ukontrollert vekst, sidan `broker` dekrypterer på KVART einaste kall uansett handling.
- **Security Auditor-pass FØR utrulling fann og fekk retta to MEDIUM-funn**: (1) den opphavlege `tenant_id`-kolonna hadde ein `REFERENCES tenants(id)`-FK, som ville fått sjølve `INSERT`-en (og dermed loggføringa) til å feile stille for eit ugyldig/oppdikta tenant_id — akkurat den mest sannsynlege forma eit lekka-nøkkel-forsøk ville teke; fjerna FK-en, tabellen er ein rein logg, ikkje eit referanseintegritets-register; (2) `service_role` mangla eksplisitt `GRANT SELECT` på den nye tabellen (same, gjentekne CLAUDE.md-funn: `service_role` får INGEN automatisk tabelltilgang) — ville blokkert Fase 2 sin komande varslingskode med ein 42501 om det ikkje var retta no. Eitt LOW-funn (manglande `NOTIFY pgrst, 'reload schema'`) retta same runde.
- **Utrulla og verifisert direkte mot `vibeverk-control`** (ikkje berre stolt på ein rein CLI-exit): stadfesta migrasjonen faktisk registrert i `supabase_migrations.schema_migrations`, `has_function_privilege`/`has_table_privilege` viser nøyaktig dei tilsikta grantsa (`service_role` ja, `anon`/`authenticated` nei), RLS aktivert, OG eit reelt testkall med ein oppdikta tenant_id — stadfesta at akkurat det Security Auditor-funnet no faktisk vert logga (tidlegare ville denne same testen forsvunne sporlaust). Testrada sletta att etterpå.
- **Fase 2/3 (varsling på `generate_support_access` + mønster-basert varsel på uvanleg mange ulike tenant_id-ar dekryptert på kort tid) er MEDVITE UTSETT** til dette har kjørt ei stund i produksjon og mønsteret kan kalibrerast mot ekte trafikk — sjå Arkitekt-forslaget i denne økta for full grunngjeving, ikkje reprodusert her.
- Ingen endring i noka kunde sitt eige dataplan-prosjekt. `test.js`/`test-workspace.js` upåverka (ingen fil i hovudrepoet endra).

## 0.159.10 — 2026-09-03

**Security Auditor-runde (ikkje knytt til éin enkelt endring): katch-up-revisjon over heile det urevisderte vindauget sidan førre pass.** Brukar spurde om det var kjørt sikkerhetsaudit i samband med dagens deploys — svaret var nei, og førre reelle Security Auditor-pass var 2026-08-19 (`6404ccf`, QR-modulen, v0.151.0), ni versjonar/to veker attende. Denne oppføringa lukkar det gapet, ikkje eit svar på eitt enkelt funn.

- **Omfang**: full diff `6404ccf..HEAD` (v0.151.0 → v0.159.9) — QR-poleringar, Arctic/AI Lab + Console-splitten (som alt hadde eit eige, meir overflatisk Security/Privacy/UX-pass ved sjølve samanslåinga 2026-08-20 — no uavhengig re-stadfesta mot faktisk kode, ikkje berre akseptert som prosa), og heile denne økta sine endringar (chat-utkast-fiks, CRM AI-kort skjult, booking/tilbud-varselbadge, biletmetadata-arbeidet i 0.159.9, og viktigast: `ANTHROPIC_API_KEY` som gjekk live for fyrste gong på begge Vercel-prosjekt).
- **Resultat: ingen BLOCKER/HIGH/MEDIUM.** Begge nye Anthropic-baserte endepunkta (`api/ai/oversikt.js`, `api/ai/annual-wheel.js`) stadfesta å verifisere tenant, innlogging, rolle OG at akkurat DEN tenanten faktisk har modulen aktivert — FØR noko Anthropic-kall skjer, slik at éin kunde ikkje kan bruke opp ein annan sin AI-kostnad. Arctic/Console-verktøya er sperra server-side (ikkje berre i UI-et) til superadmin. Biletendringane i 0.159.9 sin `C.coverImg()`-overgang escaper identisk med det dei erstatta — ingen ny XSS-flate.
- **To LOW-funn, ingen kodeendring kravd**: (1) ein Console-preview-unntak i `middleware.js` som matchar ein plattform-sett, ikkje-forfalskbar hostname eksakt — dokumentert som eit medvite avvegingspunkt, ikkje eit hol; (2) chat-widget sin nye `saveWidgetConfig()`-Supabase-skriving vart vurdert trygg ut frå migrasjonskjelda, men auditen kunne ikkje sjølv stadfeste dei faktiske grantsa/RLS-en live på produksjon utan eit databasekall.
- **Oppfølgingsverifisering (same dag, etter auditen)**: kjørte `has_table_privilege(...)` direkte mot produksjon — fann at `anon` FAKTISK har rå tabell-`GRANT INSERT`/`UPDATE` på `store` (ikkje berre `SELECT`, som auditen sin tekst hevda). Grov vidare i dei faktiske RLS-policyane (`pg_policies`) og stadfesta at `store` har RLS aktivert (`relrowsecurity=true`) og at INGEN INSERT/UPDATE/DELETE-policy på tabellen matchar `anon`-rolla i det heile — alle skrivepolicyar er avgrensa til `{authenticated}`. Nettoresultat: anonyme besøkjande kan framleis ALDRI skrive til `store` i praksis (konklusjonen frå auditen held), men den presise mekanismen er RLS-avgrensing, ikkje eit manglande tabell-grant slik auditen sin tekst beskreiv det. Retta forståing notert her for framtidige rundar — ikkje eit reelt sikkerheitshol, berre ei upresis skildring i sjølve auditrapporten.
- Uavhengig (Claude-native) pass — las aldri Codex sine eigne funn for dette vindauget først, i tråd med CLAUDE.md sitt krav om at dei to revisorane skal vere eit reelt kryss-sjekk, ikkje eitt som ekkoar det andre. Ingen kode endra som følgje av denne runda utover denne changelog-oppføringa og versjonsbumpen.

## 0.159.9 — 2026-09-03

**Bilde-metadata-kartlegging: alt-tekst eksponeres no faktisk i frontend, kredittbadge (©/KI) vist konsekvent, og helsesjekken ser alle biletmodular.** Etter kartlegging av heile Mediebank + alle andre stader bilete blir brukt, på brukar sin førespurnad (fotograf/opphaver, rettar, kreditering, alt-tekst, SEO-metadata, Google Bilete-indeksering).

- **`module-mediabank.js`**: dei to offentlege rendringspunkta (bilete-rutenett og lightbox) hardkoda `alt=""` sjølv om `App.ui.imageField()` i redigeringsskjemaet ALT let kunden skrive inn ekte alt-tekst, og verdien var lagra og tilgjengeleg via `App.media.resolveImage()`. Metadata fanst, men vart aldri vist. Retta til å bruke `img.alt`.
- **`module-faq.js`, `module-booking.js`, `module-carousel.js`**: skreiv eigne hand-rulla `<img>`-taggar i staden for den delte `C.coverImg()`-hjelparen som Hero/Om oss/Tjenester/Aktuelt/Referansar/alle tre sidemalane alt brukar. Konsekvens: eit bilete merka "Eget bilde (©)" eller "KI-generert" via det delte biletfeltet viste ALDRI kreditt-badgen på FAQ-, Booking- eller Karusell-sida, sjølv om Mediebank/Referansar/Hero viste det korrekt for tilsvarande merking. Retta ved å byte til `C.coverImg(img, "<same klassar>")` alle tre stadar — verifisert byte-for-byte at utdata er uendra når ingen kreditt er sett (kun `alt`-verdien er ny), og at kreditt-tilfellet produserer nøyaktig same `<span class="... has-credit">`-struktur som det etablerte, alt skipa mønsteret (verifisert med eit isolert jsdom-kall på `Components.coverImg()`, ikkje berre lese kode).
- **`core.js` `wchCollectImagesFrom()`/`wchCollectImages()`** (Nettsidehelse-sjekken "Alt-tekst på alle bilete", Innsikt/Design → SEO): skanna tidlegare KUN hero/about/services/news-bilete — blind for Mediebank, Karusell, Booking-ressursar, FAQ-intro og Referansar. Utvida til å ta imot eit valfritt `extra`-argument (`mediabankImages`, `carousels`, `bookingAssets`, `faqContent`, attbruker alt henta `refItems`) som fell tilbake til direkte `Store.get(...)`-lesing når det ikkje er gitt — same `opts`-mønster som `faqItems`/`refItems` alt brukte. `docs/architecture/website-health-scoring.md` oppdatert til å reflektere det nye, breiare skanne-omfanget.
- **Ikkje endra i denne runda** (forklart til brukar, lagt i `docs/roadmap/ROADMAP.md` som «innfør ved førespurnad»): strukturert fotograf/opphaver/kjelde/lisensfelt utover dagens `creditType`+fritekst-kredittlinje, utløpsdato for bruksrett, og ei eiga «Tillat indeksering i bildesøk»-innstilling (verken robots.txt eller nokon anna mekanisme gir i dag ein kunde ein måte å ekskludere eitt enkeltbilete frå Google Bilete på sjølve den offentlege Mediebank-sida).
- Ingen database- eller produksjonsendring. `test.js` (778/778) og `test-workspace.js` (303/303) grøne. Cache-bust: `module-mediabank.js` 6→7, `module-faq.js` 8→9, `module-carousel.js` 8→9, `module-booking.js` 20→21 (`index.html`+`admin/index.html`), `core.js` 97→98 (alle fire HTML-skal).
...
- **Tabellbug retta** (`console/index.html` `.cs-md-body`-CSS, `console/console-core.js` `loadLaringDoc()`): den markdown-genererte tabellen under «Trygge endringar» brukte `table-layout: auto` uten `word-break`. Den ubrutte, kodeformaterte stien `docs/architecture/customer-delivery-checklist.md` i siste kolonne kunne dermed presse tabellen breiare enn `.cs-md-body` sin `max-width: 860px`, uten scroll-mulighet. Fiksa med `table-layout: fixed`, `overflow-wrap`/`word-break` på celler og kode, og ein ny `.cs-md-table-wrap`-scrollwrapper som `loadLaringDoc()` pakkar kvar tabell i etter `marked.parse()`. Verifisert med ein isolert Playwright-rendering av same CSS/markdown (Console krev ekte OTP-innlogging og kan ikke drivast av den eksisterande `run-vibeverk`-driveren).
- **Ny SVG-arkitekturillustrasjon** i `docs/onboarding/new-team-member-onboarding.md` («Teknisk oversikt»-avsnitt, vises i Læringsdokument-fana): dei tre flatene (besøkjande/tilsette/Vibeverk-operatør) → `middleware.js` sin hostname→tenant-ruting → dataplan/kontrollplan-oppdelinga. Sjølvforsynt inline SVG (ingen ny CDN-avhengigheit, brukar dei same CSS-variablane som resten av Console). Fann og retta undervegs: CommonMark avsluttar ein rå HTML-blokk ved fyrste tomme linje, så alle tomme linjer inne i sjølve `<svg>`-blokken måtte fjernast — elles forsvann boksane/pilene stille og berre den nakne teksten vart att, verifisert med same Playwright-rendering.
- **Tre utdaterte testreferansar retta**: `new-team-member-onboarding.md`, `safe-changes-guide.md` og `customer-delivery-checklist.md` nemnde framleis berre to testpakkar (`test.js`/`test-workspace.js`) — `test-api.js` (lagt til 2026-07-27) mangla i alle tre.
- **Mobil-funn frå Claude sin UX/Mobile Reviewer, retta same runde**: `table-layout: fixed` + `width: 100%` åleine tvang tabellen sine 6 kolonnar til å dele opp `.cs-md-body` sin bredde likt på smale skjermar (under 60px per kolonne på 375px), som gjorde lesbarheita verre enn før — sjølve overflow-buggen var borte, men resultatet var no ulesleg i staden. Retta ved å leggje `min-width: 640px` på tabellen slik at `.cs-md-table-wrap` faktisk scrollar horisontalt på smale skjermar i staden for å klemme kolonnane saman. Same funn for SVG-en: faste `font-size`-verdier i SVG-koordinata vart rekna ut til å bli under 5px lesbar høgd ved full nedskalering til 375px (`width:100%` utan nedre grense). Retta med ein ny `.cs-md-svg-wrap`-scrollwrapper (`min-width: 780px` på sjølve SVG-en) og heva skriftstorleikane i SVG-koordinata (13/12/11px → 16/14/14px) som ekstra margin. Begge retta funna er stadfesta med målingar i ein isolert Playwright-rendering ved 375px viewport-breidde (`scrollable: true`, table/SVG-breidde 640/780px), ikkje berre visuell augnemål.
- Ingen database-, Supabase- eller produksjonsendring. Alle tre testsuitene grøne (sjå tal for `main` under). Cache-bust: `console-core.js` 285→286.
...
**1) Invitasjonslenker sendt frå Workspace sin eigen «Brukere»-fane enda opp innlogga direkte på det offentlege nettstedet, UTAN å nokon gong bli bedt om å setje passord.** Reell, alt-levande feil (ikkje introdusert denne runda) -- root cause stadfesta direkte mot produksjon: `uri_allow_list` i kunden sitt Supabase-prosjekt inneheldt berre det bare rot-domenet (`https://vibeverk.no/**`), aldri `workspace.<domene>`-underdomenet Workspace sin eigen invitasjons-redirectTo faktisk peikar på. GoTrue feilar ikkje synleg på ein ukjend redirectTo -- han fell stille attende til `site_url` (rot-sida), som ikkje har noko "set passord"-steg i det heile.
- Retta ved kjelda: ny delt `authAllowedUrisFor()`-hjelpefunksjon i `tenant-admin/index.ts` (kontrollplanet) inkluderer no `workspace.`/`console.`-underdomena for KVART registrert hostname, brukt av både `patchTenantAuthConfig()` (hostname-oppdatering) og `configure_tenant_smtp`.
- **Driftsmerknad:** den faktiske live-korrigeringa av Vibeverk sin eksisterande `uri_allow_list` skjer via normal deploy-og-utløys-flyt (redeploy `tenant-admin`, deretter køyr `update_tenant_hostnames` på nytt frå Console for å synke det korrigerte allow-lista) -- ein direkte ad-hoc PATCH vart avvist av tryggleiksklassifiseraren, med rette, sidan produksjons-Auth-konfig ikkje skal endrast utanfor den reviderte kodestien.

**2) To-faktor-innlogging (TOTP, t.d. Google Authenticator) for Workspace + Web-admin.** Bygd DIREKTE denne runda, utan eiga Arkitekt-/Security Auditor-runde -- eksplisitt brukarønske ("ikkje kritisk, vi reviderer når vi har meir opne credits att"), ikkje ei gløymt steg. Fail-closed-prinsippet frå OAuth-runda (0.147.0) er likevel følgt konsekvent: viss AAL-oppslaget sjølv feilar, vert brukaren ALDRI sleppt gjennom stille.
- Ny «To-faktor-innlogging»-kort i Workspace → Innstillingar (`module-settings.js`, gjeld alle roller med ein ekte Supabase-konto, ikkje berre admin): enroll (QR-kode + manuell nøkkel + stadfestingskode), av/på-status, avslåing med Tier B-varsel (forklarer konsekvensen -- passord åleine held fram å vere nok).
- Faktoren gjeld same `auth.users`-rad på tvers av Web-admin og Workspace (delt Supabase-prosjekt) -- registrer éin stad, krevst begge stader.
- Nytt, delt `mfaChallengeThenProceed()`-mønster (éin versjon i `core.js`, éin i `workspace-core.js`, ulike modal-system) kalla frå ALLE grantingsstadene i begge filene (passord, OAuth-retur/`boot()`, invitasjon/gjenoppretting) -- ikkje berre eitt av dei, elles ville nokre innloggingsvegar stille omgått kravet.
- Ingen tenant-nivå feature-flag (i motsetnad til OAuth) -- vurdert unødvendig sidan TOTP alt er sjølvmeldt PER BRUKAR og krev ingen ekstern kundeoppsett (ingen Azure/Google-app-registrering), ulikt OAuth.
- **Kjend, akseptert dekningsgap:** same grunngjeving som OAuth-runda -- verken `test.js` eller `test-workspace.js` mockar ein ekte Supabase-klient, så heile MFA-koden (enroll/challenge/verify) er utanfor jsdom-dekning. Verifisert via kodelesing + direkte Supabase-dokumentasjonssjekk av API-formene (`enroll()`/`challenge()`/`verify()`/`getAuthenticatorAssuranceLevel()`/`listFactors()`), ikkje ved faktisk manuell utprøving denne runda.

**3) Workspace sin «Personvern»-lenke: to feil retta, funne av UX/Mobile Reviewer.**
- **Innhald:** synte tidlegare HEILE den kundevendte personvernerklæringa (cookies, leads, kontaktskjema osv.), ikkje berre det som faktisk gjeld ein tilsett. Ny `priv.employeeText` (Console, `console-core.js`) publiserast no parallelt med den fulle `priv.text`, avgrensa til «employees»-blokka -- lagt til i `privacyPublicProjection()`. Ein tenant som publiserte FØR denne endringa manglar `employeeText` enno; lenka forsvinn då stille (same vakt-mønster som før) til neste publisering.
- **Visning:** modalen brukte public-site sin `terms-modal-back`/`terms-modal` (CSS for desse klassane finst BERRE i rot-sida sin `index.html`, aldri kopiert til `workspace/index.html`) -- resultatet var ein ustila, feilplassert modal klemt inn ved sida av sidemenyen, ikkje eit sentrert, dimma overlegg. Byta til Workspace sin eigen, alt-korrekte `openModal()` (Escape-handtering, fokus-felle, scroll).
- 6 nye jsdom-testar (`ah1`-`ah6`, `ai1`) stadfestar begge fiksane: lenka finst/manglar korrekt basert på `employeeText`, modalen bruker `.i-modal-backdrop` (ikkje `terms-modal-back`), og innhaldet er avgrensa til employeeText (aldri det fulle, kundevendte `.text`).
- **Ikkje testa denne runda** (kostnad/nytte-avveging, same presedens som `test-privacy-console.js` sin eigen dokumenterte utelating av `computeSupplierBlock()` sin fulle Standardforslag-sti): sjølve Console-publiseringsmekanikken (at «Publiser»-knappen faktisk skriv `employeeText` korrekt) er verifisert ved kodelesing, ikkje ein automatisert test -- forbrukssida (Workspace-visinga) er derimot grundig testa.

## 0.148.0 — 2026-08-13

**Console kan no setje opp Microsoft-/Google-OAuth for ein kunde direkte, utan å nokon gong opne Supabase Dashboard.** Arkitekt-designa (2026-08-13, sjå samtalen same dag) — vurdert som ein ny AKSJON på ei alt audita flate, ikkje eit nytt mønster: attgjenbruker konfigurasjonen `configure_tenant_smtp` (0.146-serien) alt gjer mot Supabase sitt Management API (same platformbreie token, same "stol aldri på PATCH-svaret åleine, GET-stadfest etterpå"-disiplin, same sjølvmål-sperre mot kontrollplanet sin eigen ref).

- Ny `tenant-admin`-handling `configure_tenant_oauth` (`{tenant_id, provider, client_id, client_secret, azure_tenant_id?}`), superadmin-avgrensa (same porten som alt), audit-logga til `broker_audit_log`.
- **Client secret vert ALDRI lagra i kontrollplanet** — sendast éin gong vidare til kunden sitt eige Supabase-prosjekt (der GoTrue lagrar det) og forkastast frå minnet. Skil seg medvite frå `set_tenant_service_role_key`, som faktisk lagrar sitt eige hemmelege i Vault — her er det ikkje naudsynt, sidan kunden sitt eige prosjekt alt er den varige lagringsplassen.
- Ny Console-boks i kundedetaljar («Innlogging med Microsoft/Google (OAuth)»): viser den ferdig utrekna callback-URL-en operatøren skal lime inn i Azure/Google (`https://<ref>.supabase.co/auth/v1/callback`), skjema for Client ID/Secret (+ valfri Azure Tenant/Directory-ID), og ein ✓/status-indikator (`tenants.oauth_microsoft_configured_at`/`oauth_google_configured_at`, migrasjon `20260813094945`, kontrollplanet).
- Stadfesting etter PATCH sjekkar berre `enabled`/`client_id`, MEDVITE ikkje `client_secret`-likskap — ukjent om Management API-et sitt GET-svar maskerer eller utelet hemmelege felt når dei faktisk er sette, og å påstå likskap blindt risikerer ei falsk feilmelding på eit elles vellykka lagring.
- Kolonnenivå-GRANT SELECT for dei to nye `tenants`-felta inkludert i SAME migrasjon som dei vart lagt til i — same feilklasse (manglande grant på ei kolonnenivå-tillatingsliste) har alt slått ut heile Kundar-lista to gonger tidlegare denne månaden (dpa_*-felta, `retention_policy`), retta rett frå fyrste forsøk denne runda.

**Avgrensing operatøren bør kjenne til, medvite ikkje løyst denne runda:** Management API-tokenet er FRAMLEIS platformbreitt (kan i prinsippet nå alle 4 prosjekt i Supabase-organisasjonen, ikkje berre målkunden), same avveging som `configure_tenant_smtp` gjorde i juli. Sjekka konkret om Supabase tilbaud eit ekte prosjekt-avgrensa alternativ før denne runda vart bygd — svaret er nei i praksis: prosjekt-avgrensa roller krev Team-planen ($599/mnd mot dagens Pro på $25/mnd), og sjølv då er det udokumentert om ein Personal Access Token generert av eit avgrensa medlem faktisk sjølv vert avgrensa. Reell, prisa avveging — ikkje ei gløymt oppgåve.

**Security Auditor-pass (2026-08-13, CAUTION → retta før merge):** fann at ein feilrespons frå Management API-et sitt PATCH-kall kunne enda opp med å ekkoe `client_secret` rått attende i feilteksten, som so kunne hamna uredigert i `broker_audit_log.detail` — ein tabell lesbar av KVAR aktiv operatør, ikkje berre superadmin (MEDIUM). Retta med ei enkel redigering (`.split(secret).join("[REDACTED]")`) i BÅDE den nye `configure_tenant_oauth`-handlinga og den eksisterande `configure_tenant_smtp` (same, alt-shipa mønster, funne som ein biprodukt av gjennomgangen — ikkje verdt å la stå no som det fyrste var oppdaga). I tillegg lagt til eit reelt formatkrav på `azure_tenant_id` (GUID eller eitt av Microsoft sine namngjevne multi-tenant-endepunkt) i staden for å limte det rått inn i ein URL utan sjekk (LOW).
...
- **Leverandør-/DPA-modell retta** (den viktigaste rettinga): `tba`-forklaringa ("Vibeverk AS ikkje stifta") var feil for Supabase/Resend/Plausible -- stadfesta ved å hente kvar leverandør sin eigen DPA-side direkte 2026-08-13, alle tre trer i kraft automatisk ved bruk av tenesta, uavhengig av selskapsform. Vercel er derimot eit reelt, stadfesta gap (DPA gjeld berre Pro/Enterprise, kontoen er Hobby). Ny `dpa_status`-verdi `blocked` (åtskilt frå `tba`), `VIBEVERK_VENDORS` + `vendor_registry` (migrasjon `20260813120000`) oppdatert til å reflektere dette. `computeSupplierBlock()` gatar no den kundevendte overføringsteksten på `dpaStatus`, ikkje berre `transferMechanism` -- ein "blocked"-leverandør får aldri lenger den ubetinga "dekket av SCC"-påstanden.
- **Rettar-blokka utvida**: begrensning (art. 18), protest (art. 21) og dataportabilitet (art. 20) lagt til, det ubetinga "vi sletter... uten ugrunnet opphold"-løftet mjuka opp til å reflektere GDPR sine eigne vilkår, svarfrist (normalt éin månad, EDPB-praksis) lagt til.
- **Behandlingsgrunnlag lagt til** i kontakt/tilbud/booking/chat-tekstane (rimeleg standardvurdering, ikkje eit ferdig juridisk svar -- sjå dokumentet sitt P2-punkt 1).
- **Plausible-ordlyd retta**: unngår at teksten sjølv avseier ein juridisk konklusjon om kva som er "personidentifiserbart", skildrar i staden dei konkrete tekniske faktum.
- **Behandlingsansvarleg-teksten**: "...gjennom denne nettsiden" -> "...beskrevet i denne personvernerklæringen" (dekker no Workspace/tilsette-avsnittet eksplisitt).
- **Lagringstid-plasshaldarar**: Skjematekster-fana sitt Lagringstid-felt viser no konkrete forslag (henta frå `COMPLIANCE_STANDARD_SUGGESTIONS`) som plasshaldartekst i staden for "Ikke fastsatt" -- fyller ALDRI feltet stille, berre eit betre startpunkt for operatøren.
- **`core.js` sin reserve-generator** (`computeDefaultPrivacyText()`, brukt kun for ein heilt fersk kunde FØR Personvern-fana nokon gong er opna): den gamle "Samtykke"-seksjonen (medvite fjerna frå hovudgeneratoren 2026-08-06, men gløymt her) er fjerna, rettar-avsnittet og Plausible-ordlyden retta same måte som hovudgeneratoren. Framleis ein separat, enklare funksjon by design (synkron, ingen nettverkskall) -- full samanslåing til éin generator er eit større, seinare arkitekturarbeid (P1-1 i sjekklista), ikkje del av denne runda.

Ny regresjonstest i `test-privacy-console.js` for `blocked`-statusen. `docs/compliance/personvern-full-argumentasjon-2026-08-13.md` sin P0-liste oppdatert til å vise kva som no faktisk er bygd.

## 0.145.1 — 2026-08-13

**INCIDENT: `broker`-funksjonen krasja på KVAR EINASTE kalling (inkludert CORS-preflightar), oppdaga live av brukaren via ein CORS-feilmelding i nettlesaren.** Rotårsak: `deno.land/x/imagescript@1.3.0` (brukt til biletkomprimering ved opplasting) vart tidlegare importert statisk på toppen av fila — då `deno.land` sitt CDN byrja svare med ein brotli-dekomprimeringsfeil for pakken sin WASM-ressurs, krasja HEILE funksjonen ved modullasting, stadfesta live via `function_logs` (`WORKER_ERROR`/`UncaughtException` på kvar kalling). Fiksa ved å gjere importen dynamisk og lat -- henta berre inni `compressRasterImage()` (den einaste brukaren), med URL-en bygd frå samanslåtte strengdelar (ikkje éin bokstaveleg streng) for å hindre Supabase sin bundlar frå å prøve å statisk pre-bundle han likevel (eit reelt funne bundlar-åtferd, ikkje berre teoretisk -- fyrste forsøket med ein vanleg dynamisk import feila identisk under bundling). Fell no trygt tilbake til den alt eksisterande "biletet kunne ikkje komprimerast"-feilmeldinga (400) dersom CDN-et framleis er nede, i staden for å ta ned resten av Console sin daglege config-lesing/-skriving. Deployt og verifisert direkte (OPTIONS-preflight no 200, POST med ugyldig token no ein rein 401 i staden for 500) før merge.

## 0.145.0 — 2026-08-13

**Retensjon/sletting — Fase 2 (Console-synlegheit).** Følgjer opp Fase 1 (0.144.0, kun dry-run-teljing): ny seksjon i Console → Kundar → kundedetalj, «Automatisert sletting av gamle kontaktskjema-leads», som (1) let ein operatør skru PÅ/AV dry-run-teljing per kunde og setje talet på månader (default 12), og (2) viser siste `retention_runs`-rad for kunden («Sist kjørt: … — N kandidatar funne»), henta direkte frå kontrollplanet (ingen ny `tenant-admin`-handling trengst for LESING, RLS tillet det alt). Ny `tenant-admin`-handling `set_tenant_retention_policy` for SKRIVING (same opne tilgangsnivå som DPA-handlingane — kvar aktiv operatør, ikkje superadmin-avgrensa, sidan brytaren framleis berre styrer TELJING, aldri sletting). **Framleis ingen kodeveg for faktisk sletting** — Fase 3 er ein eigen, seinare kodeendring som krev Security Auditor-pass. Fanga og retta FØR utrulling (ikkje etter, denne gongen): `retention_policy`-kolonnen (lagt til i Fase 1) hadde same manglande kolonnenivå-grant som utløyste 0.143.1-hendelsen — retta i eigen migrasjon (`20260813090000_grant_retention_policy_authenticated.sql`) FØR `loadTenants()` byrja å spørje etter kolonnen, ikkje etter ein ny produksjonsfeil.

## 0.144.0 — 2026-08-12

**Retensjon/sletting — Fase 1 (grunnmur, kun dry-run, INGEN sletting mogleg enno).** Bevis-leveranse for det compliance-revisjonsdokumentet (`docs/compliance/personvern-rammeverk-status-2026-08-12.md`, del 2) peika på: skrivne slettefrister utan noka faktisk automatisert handheving. Arkitekt-planlagt før bygging (nytt cross-project-mønster). Ny Edge Function `retention-sweep` i `vibeverk-control` — gjenbrukar broker sitt vault+service_role-kryssprosjektmønster, men er heilt separat frå `broker`/`tenant-admin` (ingen operatør-autentisering, kalla av eit sentralt pg_cron-job i staden). Tel kandidat-leads (`created_at` eldre enn tenanten sin `retention_policy->leads->months`, default 12) for kvar aktiv tenant med `retention_policy->leads->enabled = true` (default `false` for ALLE kundar — eksplisitt opt-in, ikkje ein big-bang), skriv talet til ny `retention_runs`-tabell (RLS: operatør-lesbar, kun service_role-skrivbar, speglar `broker_audit_log`). Ny `tenants.retention_policy jsonb`-kolonne — deliberately IKKJE same felt som den eksisterande fritekst-`sc.privacy.forms.kontakt.retention`-strengen (den er visningstekst for personvernerklæringa, ikkje handhevingsgrunnlag). Ny migrasjon i kvart kundeprosjekt (`supabase/migrations/`): eksplisitt `GRANT SELECT, DELETE ON leads TO service_role` (service_role har ingen automatiske tabellrettar, stadfesta fallgruve ADR-0009/CLAUDE.md). `pg_cron`+`pg_net` aktivert i `vibeverk-control` (kun der), dagleg kl. 03:17 UTC via ein `SECURITY DEFINER`-funksjon som les kontrollplanet sin eigen service_role-nøkkel frå Vault under namnet `control_plane_service_role_key` — denne må setjast manuelt via `vault.create_secret(...)` (aldri i ein git-sporet fil, same disiplin som per-tenant-nøklane) før jobben faktisk gjer noko; fram til då feilar han stille og trygt. **Ingen kode i denne versjonen kan slette ei einaste rad** — Fase 3 (faktisk sletting, kun for éin bevis-kunde med eksplisitt `enabled=true`) er ein eigen, seinare kodeendring som må gjennom Security Auditor før utrulling. Fase 2 (Console-visning av sweep-resultat) og Fase 3 er ikkje bygd enno.
...
- **Ekte bug retta**: "Legg til blokk"-knappen kunne bare klikkes én gang før operatøren måtte minimere/åpne seksjonen på nytt for å legge til enda en blokk. Rotårsak: knappen lå UTENFOR DOM-regionen som ble tegnet på nytt etter hvert legg-til/fjern/flytt, så hver `renderBlocksEditor()`-kjøring la på enda en "click"-lytter oppå den samme, aldri-fjernede knappen. Andre klikk fyrte to lyttere synkront etter hverandre — den ene åpnet type-velgeren, den andre lukket den igjen i samme handling, så ingenting så ut til å skje. Løst ved å flytte "Legg til blokk"-knappen (og type-velgeren) INN i den regionen som faktisk tegnes på nytt hver gang — da blir de destruert og skapt fullt på nytt hver runde, og lyttere kan aldri hope seg opp.
- **Kolonner er nå tydelig gruppert**: med et flerkolonne-oppsett (f.eks. "3 kolonner") viste editoren tidligere bare én flat liste + én generisk "Legg til blokk"-knapp som alltid la nye blokker i kolonne 1 — ingen tydelig måte å se/fylle hver av de N kolonnene. Hver kolonne får nå sin egen, tydelig avgrensa boks med overskrift ("Kolonne 1", "Kolonne 2" osv.), en egen "Legg til blokk"-knapp som legger den nye blokka RETT i den kolonnen, og en tydelig tom-tilstand når kolonnen ikke har noe innhold ennå. "Kolonne"-nedtrekksmenyen på hvert kort står igjen for å flytte en eksisterende blokk til en annen kolonne senere.
- **Ny valgfri "Ramme inn (bakgrunn og kant)"-avkryssing** på hver blokktype (unntatt mellomrom) — samme visuelle behandling som `.pb-grid__item` allerede har (bakgrunn, kant, avrundede hjørner, padding). Av som standard, bakoverkompatibelt med allerede lagrede blokker.
- Ingen Security Auditor-runde denne gangen (ren UI-restrukturering + boolsk CSS-toggle, ingen ny tekst-/href-/CSS-interpoleringsflate) — men en UX/Mobile Reviewer-runde ble kjørt gitt hvor mye editor-UI-et endret seg. Den fant ett HIGH-funn (retta før merge): å endre ei EKSISTERANDE blokk sin «Kolonne»-veljar lagra riktig i dataen, men flytta aldri kortet visuelt til den nye kolonne-boksen — akkurat den typen stille ikkje-effekt denne runda skulle fjerne. Retta ved at slot-veljaren no utløyser ei full re-rendring, ikkje berre den generiske felt-endrings-handteraren. Også retta eit MEDIUM-funn: fleire kolonner sine «Legg til blokk»-pickerar kunne stå opne samstundes (før fanst det berre éin picker i heile seksjonen) -- lukkar no alle andre opne pickerar før ein ny vert opna.
- 2 nye regresjonstestar låser fast dobbelt-lytter-buggen (legge til to blokker på rad uten å måtte minimere seksjonen mellom hvert klikk) og kolonne-veljar-re-rendringa, pluss tester for kolonne-gruppert plassering og ramme-avkryssingen.
...
- **Ekte live forhåndsvisning** -- Sider-fanens redigeringsvisning fikk et nytt to-panels arbeidsområde: seksjonsliste til venstre, en `<iframe>`-basert forhåndsvisning til høyre som kaller `C.pageSection()` DIREKTE (samme, ekte rendringsfunksjon som `module-page-builder.js` bruker på den faktiske offentlige siden) og oppdaterer seg umiddelbart mens man skriver. Iframen fylles med tenantens FAKTISKE farge-/font-valg fra superconfig når de finnes, ikke bare nøytrale standardverdier. Bevisst en `<iframe>`, ikke en vanlig innsnevret `<div>` -- CSS-medieforespørsler reagerer på selve vinduet sin bredde, en tidligere mockup-variant med en smalnet div viste derfor feilaktig et ustablet rutenett på "mobil"-forhåndsvisning (se skjermbilde i samtalen); iframen sin egen bredde trigger nå de virkelige bruddpunktene korrekt.
- **Dra-og-slipp reordning** av seksjonskort (native HTML5 DnD), med opp/ned-piler beholdt som tastaturtilgjengelig fallback. Brukeren spurte spesifikt om dra-og-slipp direkte i selve forhåndsvisningen (Wix/Squarespace-stil) -- vurdert og bevisst utsatt: det krever at forhåndsvisningen blir en fullt interaktiv redigeringsflate (treff-testing av vilkårlig rendret HTML per seksjonstype), en vesentlig større investering enn listebasert dra-og-slipp med umiddelbar forhåndsvisning ved siden av.
- **Inline-ekspanderende seksjonskort** (ett akkordeon-kort per seksjon, med typeikon og en live ett-linjes oppsummering av det faktiske innholdet) erstatter den tidligere separate, globale redigerings-boksen -- hele siden er nå synlig samtidig som man redigerer én seksjon.
- **Ikonbasert typevelger** ("Legg til seksjon") erstatter en bar `<select>`.
- **Autolagring** (debounced 700ms) på hvert feltendring erstatter en eksplisitt "Lagre seksjon"-knapp -- matcher den validerte mockupens flyt.
- **Proaktive, handlingsrettede feilmeldinger for bildeopplasting**: størrelses-/formatgrensene (PNG/JPEG opptil 8MB, komprimeres automatisk til 600KB; SVG/WebP opptil 600KB rått) vises nå i feltets hint-tekst FØR noen fil velges, og en avvist opplasting gir et konkret forslag ("prøv lavere oppløsning" / "bruk PNG/JPEG i stedet"), ikke bare en generisk avvisning -- direkte svar på brukerens eksempel ("F.eks. var .png bilde for stort").
- Sidelisten fikk en liten typeikonrad per sidekort for raskere oversikt; sidesletting fra selve redigeringsvisningen (ikke bare listen) lagt til som en egen "Slett side"-knapp i toppen.
- Nye tester: 3 nye i `test-page-builder-console.js` (dra-og-slipp, sletting fra redigeringsvisningen, proaktiv/handlingsrettet feilmelding ved for stort bilde), pluss et frittstående integrasjonssøk som laster den EKTE `components.js` (ikke en test-stub) for å bekrefte at den nye iframe-forhåndsvisningen faktisk produserer korrekt, ekte markup.
- Karusell-idé (bildekarusell som egen seksjonstype, mulig gjenbruk av eksisterende `module-carousel.js`) notert som god fremtidig utvidelse, ikke del av denne runden.
- **Security Auditor-funn FØR merge (BLOCKER, uavhengig gjennomgang): lagret XSS i selve forhåndsvisningen, som kunne eskalert en kunde-admin til å stjele en ekte Console-operatørs plattformbrede økt.** `sc.colors`/`sc.fonts` (superconfig, fritt settbart av en kunde-admin via egen Web-admin -- et rent fritekstfelt uten allowlist) ble limt uendret inn i en CSS-streng som gikk gjennom `doc.write()` -- en ekte HTML-parser, ikke `innerHTML`. En font-/fargeverdi som inneholdt `</style><script>...` kunne dermed bryte ut av `<style>`-taggen og kjøre inne i iframen, som ikke hadde noen `sandbox` og derfor delte samme opphav som Console selv -- inkludert tilgang til `_sbControl` sin `persistSession:true`-token i `localStorage`. Rettet med to uavhengige lag: (1) streng allowlist for farge-/fontverdier (`pbSafeCssColor`/`pbSafeCssFontName`, faller til nøytrale standardverdier ved avvik), (2) `sandbox="allow-same-origin"` (ALDRI `allow-scripts`) på selve `<iframe>`-en, som gjør at ingen skript kan kjøre der i det hele tatt, selv om en fremtidig ny superconfig-verdi skulle slippe usanert gjennom ved en feil. 3 nye tester låser begge lagene fast, inkludert et nytt `VwConsole._test`-eksponeringspunkt (samme mønster som `core.js` sin egen `_test`) for å teste `pbPreviewCss()` direkte.
- UX-funn FØR merge: ingen visuell skille mellom "lagrer"- og feilet-autolagring-status (lagt til egen `.is-error`-klasse); forhåndsvisningen havnet under en potensielt lang seksjonsliste ved smalere Console-vinduer, ikke bare telefon (rettet med CSS `order` slik at forhåndsvisningen vises først når arbeidsområdet går til én kolonne); rutenett-ruter og seksjonslisten kunne vokse ubegrenset høye (fikk egne rullefelt med en høydegrense som matcher forhåndsvisningsruten); mobilforhåndsvisnings-knappene manglet en breddeetikett; drag-håndtak og mini-handlingsknapper var under vanlig trykkmål (økt fra 28px til 32-36px, siden Console også brukes fra nettbrett i praksis).

## 0.133.0 — 2026-08-11

**Sidebygger, Fase 1 (Console-only): kontrollert seksjonsbygger for kundeekstrasider.** Bruker etterspurte en generisk måte å levere rikere ekstrasider ("Om oss" mer inngående, "Jobb hos oss", "Ansatte", HMS, sertifiseringer) uten å måtte spesialkode hver enkelt i den delte kodebasen. Etter diskusjon (inkl. gjennomgang av et ChatGPT-forslag brukeren delte) landet vi på: generaliser byggeklossene, ikke de ferdige sidene. Arkitekt-konsultert før implementering.

- Åtte kontrollerte seksjonstyper (`components.js`: `pageSection()` + `pbHero/pbText/pbImageText/pbBigImage/pbQuote/pbGrid/pbCta/pbSpacer`) -- ingen fri HTML/CSS/JS, ingen pikselplassering. `pbGrid` (1-4 kolonner, hvert element uavhengig valgfritt bilde/overskrift/tekst/knapp) er den generaliserte arbeidshesten -- samme mekanisme dekker ansattkort, tjenestekort, bildegalleri, referanser, sertifiseringer og "hvorfor oss"-punkter, i stedet for en smal type per behov.
- Ny `module-page-builder.js`: leser `"custom-pages"`-nøkkelen (samme `store`-mønster som `module-scrollbanner.js`) og registrerer én `page:true`-modul per lagret side -- egen URL/nav som Mediabank/Aktuelt, ikke innbakt på forsiden i Fase 1 (datamodellen sperrer ikke for det senere). Tilgjengelig for ALLE tenanter (ikke låst bak det eksisterende betalte `features.sidebygger`-flagget -- Fase 1 er uansett Console-drevet, ikke kundens eget verktøy).
- `module-sidetelling.js` sin statiske `REAL_PAGE_PATTERNS`-liste utvidet med en dynamisk sjekk mot `custom-pages` direkte, siden operatør-opprettede sider ikke kan forhåndskodes én regex om gangen.
- Ny Console-fane "Sider" (`console-core.js`): sideliste, opprett/rediger/slett side, legg til/rediger/slett/flytt opp-ned seksjon, type-spesifikke redigeringsfelt, egen bildeopplasting via ny broker-handling `upload_section_image` (samme service-role-mønster som `upload_logo`, men kalibrert for fullbreddebilder: 600KB/8MB i stedet for 300KB/6MB). `"custom-pages"` lagt til broker sin `ALLOWED_CONFIG_KEYS` (nødvendig -- Console sin egen tenant-klient er anon/uten sesjon og kan aldri tilfredsstille `store` sin `can_edit_content()`-RLS).
- Nye tester: 19 nye assertions i `test.js` (alle åtte seksjonstyper, ukjent-type-fallback, sletta-side-fallback siden `App.registerModule()` ikke har noen unregister-mekanisme, `navHidden`), ny `test-page-builder-console.js` (9 tester, samme jsdom+`node:test`-mønster som `test-customer-analysis-console.js`), pluss én ny assertion i den isolerte sidetelling-testen som bekrefter en dynamisk sidebygger-rute faktisk telles som sidevisning.
- Fase 1 avgrenset med vilje: ingen dra-og-slipp (Flytt opp/ned i stedet), ingen kladd/publisert-flyt, ingen kundeselvbetjening (kommer i Fase 2, gated på et allerede-bygget `locked`-felt per side), ingen forsideinnbaking ennå, ingen "dynamisk liste"-seksjonstype for et fremtidig ansatte/stillinger-register (de bør bli egne strukturerte moduler som CRM, ikke bakes inn i seksjonsbyggeren selv, hvis/når det behovet blir reelt).
- Security Auditor-funn FØR merge (MEDIUM, uavhengig gjennomgang): de tre nye knapplenke-feltene (hero/CTA/rutenett) tok imot fritt skrevet URL fra en Console-operatør og sendte den uendret til `button()` i `components.js` -- `esc()` alene hindrer ikke en `javascript:`-URL, kun HTML-spesialtegn. Samme farerklasse som `sanitizeRichHtml()` sin egen `<a href>`-sanering og `module-quote.js`/`module-crm.js` sin `isSafeAttachmentUrl()` (begge bruker identisk regex) -- men de tre nye stiene var de eneste stedene i denne runden en fritt-skrevet URL nådde `button()`s `href` uten vakt. Rettet ÉN gang, i `button()` selv (ikke i hver kaller): en `javascript:`-URL blir nå aldri et ekte `href`, knappen faller tilbake til et vanlig `<button>` med teksten intakt. Ny test i `test.js` låser fiksen fast.
- UX-funn FØR merge: (1) ingen advarsel noe sted om at Sidebygger i Fase 1 ikke har kladd/forhåndsvisning -- alt en operatør lagrer er umiddelbart synlig for besøkende. Lagt til som en Nivå B-inline-advarsel (samme mønster som sidesperre-kortet) øverst i Sider-fanen. (2) Flytt opp/ned brukte rå "↑"/"↓"-tegn som helt UTELOT seg selv ved listegrensa, ulikt Priser sin identiske pakke-flytting (`ti-chevron-up/down` + `disabled`) -- rettet til samme mønster, unngår at knapperada endrer bredde avhengig av posisjon.

## 0.132.1 — 2026-08-11

**Rettet: sidesperre AV falt tilbake til den globale sperra, i staden for å opne domenet heilt.** Bruker meldte feil rett etter fyrste faktiske bruk: å slå AV den nye per-tenant-sperra (0.132.0) let framleis den delte utviklingssperra (`SITE_LOCK_PASSWORD`) gjelde for domenet -- brukeren forventa "AV = heilt av".

- Problemet: eit reint `site_lock_enabled`-flagg kan ikkje skilje "aldri konfigurert for denne tenanten" frå "operatør har eksplisitt slått AV" -- begge er `false`. Sidan `false` er standardverdien for ALLE tenantar (også dei som aldri har rørt funksjonen), ville "AV = heilt open" bokstavleg tolka ha fjerna den globale utviklingssperra for HEILE plattformen med det same.
- Løysing (brukervedtak same dag, tre tilstandar): ny migrasjon `20260811074128_tenant_site_lock_off_state.sql` legg til `tenants.site_lock_ever_enabled` -- eit **monotont** flagg (kan berre gå frå `false` til `true`, aldri attende), sett av `set_tenant_site_lock()` berre når `p_enabled = true` faktisk vert sendt inn. Eksponert i `resolve_tenant_by_hostname()`. `middleware.js` sin lock-avgjerd: `site_lock_enabled=true` → tenantens eige passord; `site_lock_enabled=false` men `site_lock_ever_enabled=true` (sperra HAR vore PÅ minst éin gong før) → domenet er HEILT opent, ingen sperre i det heile; `site_lock_ever_enabled=false` (aldri PÅ) → uendra, fell attende til den delte utviklingssperra.
- Security Auditor-funn FØR merge (HIGH, uavhengig gjennomgang): eit FYRSTE forsøk brukte `site_lock_updated_at` (settast av eit kvart passord-lagre, uavhengig av om "Sperre PÅ" var kryssa av) som skiljet i staden. Det ville late ein operatør som berre "legg inn eit passord for seinare" -- utan å krysse av boksen -- ved eit uhell opne eit live kundedomene HEILT, forbi både eit ikkje-eksisterande tenant-passord OG den globale sperra. Retta ved å byte til det monotone `site_lock_ever_enabled`-flagget, som berre kan bli sant etter ei EKTE PÅ-hending.
- Console-korta sin tekst og statusetikett ("PÅ (eige passord)" / "AV (heilt open)" / "aldri konfigurert (bruker delt utviklingssperre)") oppdatert til å forklare dette eksplisitt, inkludert i den aktive-tenant-åtvaringa og eit eksplisitt "passord lagra utan å krysse av → ingenting opnar seg"-punkt.
- Fann og retta samtidig ein latent testfeil i `test-api.js` sitt d11-scenario ("feila verify-RPC failar CLOSED") -- mocken sette aldri `site_lock_enabled=true` for det scenarioet, så testen passerte av feil grunn (via den globale sperra, ikkje den faktiske fail-closed-koden). Nye tester d12-d13 dekker den nye AV-tilstanden direkte.
- Krev ny Edge Function-deploy av `tenant-admin` og ny migrasjon mot `vibeverk-control` for å bli reelt aktiv (same som 0.132.0 -- Supabase Edge Functions redeployer ikkje automatisk ved `git push` slik Vercel-sida/Console gjer).
...
- Ny migrasjon `supabase-control/supabase/migrations/20260810190000_add_customer_analysis.sql` med analyser, kjøringer/leaser, sider, funn, tjenestekoblinger, møtegrunnlag og hendelseslogg. Aktive operatører har RLS-beskyttet lesing; nettleseren har ingen direkte skrivetilgang. Atomisk `claim_customer_analysis_run()` håndhever én kjøring per domene og tre globalt.
- SSRF-vern med full DNS-validering og faktisk IP-pinning i Node-socketen, peer-adressekontroll og samme kontroll etter hver manuelle redirect. Private/reserverte IPv4/IPv6-mål, interne navn, IP-litteraler, legitimasjon, uvanlige porter og HTTPS-til-HTTP-nedgradering blokkeres. Streng robots-håndtering, identifiserbar User-Agent, femsidegrense, byte-/redirect-/request-/total-tidsgrenser.
- HTML-kontroller for teknisk kvalitet, SEO, maskinelt kontrollerbar universell utforming, personvern-/tillitsindikatorer og grunnleggende innhold. Hele HTML-sider lagres ikke; bare tekniske resultater og korte tekstutdrag.
- Valgfri Anthropic-del følger produksjonsmønsteret med Node runtime, miljøstyrt modell, forced tool-use og servervalidering. Funn uten gyldig snapshot-URL og ordrett kildeutdrag forkastes. AI-feil lar tekniske resultater fullføres.
- Console fikk oversikt/søk/filter, oppretting, fremdrift, resultatfaner, funnreview, manuelle funn, tjenestekatalog og møtegrunnlag. Redigerte funn må godkjennes eksplisitt etterpå. Ingen kundevendt Workspace-funksjonalitet er endret.
- Ny runtime-avhengighet: `parse5@^8.0.1` for standardbasert HTML-parsing. 22 testtilfeller i `test-customer-analysis.js`/`test-customer-analysis-console.js` (uendret av reviewrunden), pluss uendra `test.js`/`test-workspace.js`/`test-api.js`. Alle bruker mocks, ingen ekte eksterne eller betalte kall.
- **Uavhengig Security Auditor (Claude-native, las aldri Codex sin eigen rapport først)**: ingen BLOCKER/HIGH. To LOW retta i denne runden: `isSafeCandidate()` mangla eksplisitt protokollsjekk (ikkje reelt utnyttbart, sidan `secureFetch` uansett avviste alt anna enn http/https lenger nede, men retta for konsistens), og `walk()` i HTML-parsaren hadde ingen rekursjonsdjupnegrense (kunne kaste `RangeError` på tilstrekkeleg nøsta HTML -- vart alt fanga trygt som ein feila køyring, ikkje eit krasj, men no unngått heilt med eit djupnetak på 400).
- **Privacy and Compliance Advisor**: ingen PII i sjølve kjeldevalet, men `textExcerpt` er ikkje PII-filtrert og sendast rått til Anthropic når AI-vurdering er slått på. **Det finst i dag ingen retensjons- eller sletterute** utover arkivering (som berre endrar status, ikkje slettar) -- identifisert som den klaraste, konkrete lanseringsporten. DPA-dekning, overføringsgrunnlag til Anthropic og rettsleg grunnlag for å analysere eit prospekt utan samtykke er korrekt dokumentert som ope i `docs/compliance/data-map-vibeverk.md`, ikkje stille føresett løyst. Ingen compliance-påstand gjort.
- **UX and Mobile Reviewer**: 3 BLOCKER + 3 HIGH funne, alle retta i denne runden. BLOCKER: «Kjør på nytt» gjorde alt godkjent/redigert gjennomgangsarbeid frå førre køyring usynleg utan åtvaring (lagt til eksplisitt stadfesting); `kaRun()` mangla same staleness-vern som `kaLoadDetail()` alt hadde, slik at ei seint-løyst køyring for éin analyse kunne overskrive skjermen til ein operatør som i mellomtida hadde navigert til ein heilt annan analyse (lagt til identisk vern); automatisk/ikkje-vurdert-tilstandane (den viktigaste å skilje ut, sidan det betyr «framleis ukontrollert AI-utdata») delte visuelt same nøytrale standardfarge som alt anna (nye `.ka-badge--automatic`/`--unreviewed`/`--medium`/`--low`-klassar). HIGH: kvart Godkjenn/Fjern-klikk gjorde eit fullt nettverksoppslag + spinner-blink + omteikning av heile detaljvisinga for éi enkelt statusendring (Godkjenn/Fjern patcher no lokal tilstand og teiknar om utan nytt oppslag; redigering held fram med full reload sidan fleire felt+tenestekoplingar endrar seg samstundes); ingen framdriftskjensle under ei fleire-minutt-lang køyring (lagt til ei enkel, klientside-tidsstyrt steg-tekst); «Arkiver»-stadfestinga antyda meir reversibilitet enn det finst ein reell veg attende for i UI-et (presisert i teksten). 4 MEDIUM/nokre polish-funn dokumenterte som akseptert for denne runden (upresis «Sikkerhet»-nemning på AI-tillitsgrad retta i same slag sidan ho var billig; touch-mål under 44px på `.btn--sm` er ein sidedekkande, ikkje-ny Console-klasse).
- Dokumentasjon: `docs/architecture/customer-analysis.md`, arkitekturindeks, CURRENT_STATE og compliance-datakart oppdatert (av Codex).
- **Framleis ikkje deployet**: migrasjonen er ikkje køyrd mot `vibeverk-control`, `VIBEVERK_CONTROL_SERVICE_ROLE_KEY` er ikkje sett, og ingen deploy/push til produksjon er gjort. Attståande før staging: køyr migrasjonen mot `vibeverk-control` (krev eksplisitt godkjenning), set servermiljøvariabelen, etabler ein reell retensjons-/sletterutine, og avklar DPA/rettsleg-grunnlag-spørsmåla over -- ingen av desse er gjort i denne runden.
- Cache-bust: `console-core.js?v=237` (uendra frå Codex sin versjon -- reviewrunden sine JS-endringar går inn i same fil/versjon).
...
- Retta ein reell versjonsregresjon (`VIBEVERK_VERSION` var sett attende frå 0.124.0 til 0.109.0) og ein CHANGELOG-regresjon (uncommitta endring hadde erstatta rundt 224 linjer nyare historikk med ein eldre 0.109.0-post) -- begge oppstod fordi AI Lab-arbeidet opphavleg vart gjort mot eit eldre `main`, medan resten av plattforma gjekk vidare til 0.124.0 medan det låg uncommitta. CURRENT_STATE.md sin "Last verified"-header vart tilsvarande attsett til HEAD sin faktiske verdi; berre éin ny, tilleggande AI Lab-oppføring vart lagt til, ingen nyare fakta rulla attende.
- Fiksa ein reell HTTP-lagsfeil: `AI_LAB_TRUNCATED_RESPONSE` (kasta av Ollama-adapteren når konteksten vart avkorta) mangla frå `safeError()`-allowlista i `ai-lab-server.js`, og vart difor stille degradert til ein generisk 500-feil i staden for den tiltenkte, saniterte 502-meldinga. Lagt til allowlista + ny HTTP-regresjonstest.
- Arkitekt-konsultert (per standardregelen for kontraktsendringar) og implementert ein hardare review-kontrakt (`learning-review-v1` → `learning-review-v2`): reviewaren (Haiku) må no gje eit eige verdict (GODKJENT/MÅ RETTES/MANGLER KILDE) for kvar av `moduleDescription`, `howItWorks`, `onboardingText`, `notDocumented`, og éin indeksert verdict per quiz- og kontrollspørsmål. Servervalidatoren gjer det no strukturelt umogleg å returnere eit samla GODKJENT med mindre ALLE delvurderingar er GODKJENT -- lukkar ein reell feil der Haiku tidlegare kunne godkjenne heile utkastet utan å nokon gong eksplisitt vurdere onboarding-avsnittet.
- Uavhengig Security Auditor (2 HIGH, begge retta): ei case-sensitiv filnamn-sperre i den statiske utviklingsserveren kunne omgåast på case-insensitive filsystem (macOS/Windows) med t.d. `/SCRIPTS/`-stig -- retta til å samanlikne små bokstavar, ny regresjonstest lagt til. Flagga at ein ekte Anthropic-nøkkel i `scripts/ai-lab.env.local` (korrekt gitignora og chmod 600, aldri staged/committa) vart lese av sjølve revisjonsøkta -- tilrådd rotert som eit forsiktig tiltak.
- Privacy and Compliance Advisor: ingen personopplysningar funne i dei ti allowlista kjeldefilene i dag; det einaste realistiske innslagspunktet for personopplysningar er det frie instruksjonsfeltet, som UI-hintet alt eksplisitt åtvarar mot. Opne punkt som ikkje kan stadfestast frå kodebasen er dokumentert: Anthropics DPA-dekning, retensjon, datasenterregion/overføring, og om reell (ikkje-mocka) bruk av fleire enn éin tilsett treng eit internt godkjenningssteg.
- UX and Mobile Reviewer (3 HIGH, alle retta): `.i-notice`/`.i-notice--warn` mangla CSS-definisjon heilt og rendra som ustyla tekst -- inkludert den viktigaste åtvaringa om at data forlèt maskinen til Anthropic -- lagt til éin delt CSS-regel. Nytt review-panel-innhald mangla `overflow-wrap` samanlikna med søsken-element -- retta. "Kontekstvindu"-sjargong i eit brukarvendt hint bytt ut med eit klart konsekvens-utsegn, per `docs/architecture/copy-style-guide.md`. 4 MEDIUM- og 5 polish-funn (skanbarheit i eit no lengre reviewpanel, tvitydig gjenbruk av "IKKE DOKUMENTERT"-overskrifta i reviewet, manglande fullført-annonsering for skjermlesar, manglande passord-reveal-toggle på tilgangstoken-feltet, m.fl.) er dokumenterte, men ikkje retta i denne runden -- ingen BLOCKER/HIGH blant dei, og dette er eit internt localhost-only utviklarverktøy, ikkje ei kundevend flate.
- Test: `scripts/local-ai-client.test.js` 32/32, `test-ai-lab.js` 14/14 (nye: HTTP-lags-regresjon for avkorta-feilen, case-insensitiv denylist-regresjon, seks nye assertions for den nye review-kontrakten), `test-ai-lab-console.js` 4/4 (ny: fullt review-panel med delvurderingar, ikkje berre draft-visinga -- panelet hadde ingen eiga dekning frå før). Uendra plattformtestar: `test.js` 733/733, `test-workspace.js` 296/296, `test-api.js` 109/109 -- stadfestar at AI Lab-integrasjonen ikkje rørte noka anna Console-, Workspace- eller sideutfunksjonalitet.
- Framleis strengt localhost-only og ikkje deployert: `NODE_ENV=development` + `AI_LAB_ENABLED=true` kravd, hard sperre mot CI/Vercel/miljøproxy, ingen database, ingen automatisk publisering. Landa på ein eigen feature-branch (`feature/local-ai-lab`) via draft PR -- IKKJE merga til `main`, ingen deploy.
- Cache-bust: `console-core.js?v=236`.
...
- `module-sidetelling.js` les `utm_source`/`utm_medium`/`utm_campaign` frå landingssida sin spørrestreng (`location.search`), trimmar og kappar kvart felt til 100 teikn, og sender dei berre på `type:"pageview"` (aldri på `cta`) — same landingsside-berre-logikk som `referrer` alt brukar. To nye topplister ("Kampanjekilder"/"Kampanjer") i Kilder & enheter-fana, synlege berre når data faktisk finst.
- Ny migrasjon `20260807062325_add_analytics_utm_tracking.sql`: tre nye nullbare `text`-kolonnar med lengde-CHECK (≤100), og ein ny 10-argument `insert_analytics_event_service()`-signatur (same `DROP FUNCTION IF EXISTS`-mønster som 20260803140201 sjølv innfører, for å unngå ein usynleg overlasta funksjon ved sida av den gamle). Same rollesjekk/kvotelogikk (gruppe-200/minutt-60/dag-10000) frå 20260806170936 er uendra vidareført.
- **Uavhengig Claude Security Auditor-pass, éin LOW-funn retta før merge**: `validUtmValue()` i Edge-handteraren filtrerte opphavleg berre ASCII-kontrollteikn, ikkje Unicode bidi-/format-overstyringsteikn (t.d. RTLO U+202E) — ingen HTML-injeksjonsveg (`esc()` nøytraliserer uansett `< > & " '`), men eit slikt teikn kunne visuelt forvrengje/forfalske korleis ein admin les ein kampanjeverdi i topplista. Retta med ein utvida regex (`\u200E \u200F \u202A-\u202E \u2066-\u2069`). SQL-injeksjon vart eksplisitt vurdert og avkrefta (RPC-kallet er alltid parameterisert, ingen `EXECUTE`/`format()`/strengkonkatenering i funksjonskroppen).
- **Uavhengig UX/Mobile-pass, éin HIGH-funn retta**: den gamle `auto-fit, minmax(230px,1fr)`-grida for kort kunne la eit 4. kort (Kampanjer) hamne åleine på ei eiga rad med tomt rom attmed, akkurat ved dei ~1024-1040px breiddene som er det nemnde skrivebord-minimumet å teste mot. Retta med ei fast 2-kolonne-grid + ein generisk "siste-og-åleine-kort spenner heile rada"-CSS-regel (`:last-child:nth-child(2n+1)`), stadfesta empirisk med reelle Playwright-skjermbilete ved 2/3/4 kort på 1024px og 390px, ikkje berre rekna ut. Éin MEDIUM-funn òg retta: dei to nye korta hadde identisk hjelpetekst -- fekk no kvar sitt korte, distingverande tillegg (kva ein "kilde" vs. ein "kampanje" faktisk er).
...
## Driftsnotat — 2026-08-07: header-tillit for sidetelling-event er empirisk stadfesta

Før noko av server-side hash-ombygginga (branch `sidetelling-server-side-hash`) vart deploya til produksjon, vart det siste opne punktet frå Security Auditor sitt HIGH-funn (sjå 0.106.0-oppføringa under) verifisert direkte mot vibeverk-staging: eit mellombels, throwaway diagnostikk-endepunkt (`diag-headers-tmp`, ekkoa berre rå request-headerar attende, sletta same dag straks verifiseringa var gjort — ingen spor att i koden) synte at:

- `x-forwarded-for` vert HEILT OMSKRIVE av Cloudflare-edgen framfor Supabase til den ekte proxy-kjeda — eit forfalska klient-verdi (`X-Forwarded-For: 1.2.3.4`) vart aldri bevart, korkje som fyrste eller seinare ledd i kjeda.
- `cf-connecting-ip` er reelt IKKJE klient-forfalskbar: eit forsøk på å setje han sjølv (`CF-Connecting-IP: ...`) resulterte i at Cloudflare avviste HEILE requesten (`403`, feilkode 1000) — aktivt blokkert, ikkje berre ignorert.
- `x-real-ip` vert strippa heilt vekk, uansett kva klienten sender.

**Konklusjon**: den opphavlege `requestIp()`-koden sin trust-modell var trygg heile tida på denne konkrete infrastrukturen. Security Auditor sitt HIGH-funn var ei korrekt, forsiktig føre-var-vurdering gjort UTAN denne stadfestinga (rett å flagge det då), ikkje ein reell, utnytta sårbarheit i praksis. Minutt-kvota (`analytics_event_minute_quota`) vert likevel verande i koden som eit uavhengig, framleis nyttig forsvar mot ei ANNA trusselklasse (eit ekte volum-åtak frå mange faktiske, ikkje-forfalska IP-ar) — ikkje fjerna berre fordi denne spesifikke spoofing-vegen synte seg stengd.

Dette var det siste attverande punktet frå denne fasen sin eigen "MUST verify before deployment"-liste. Sjå `docs/architecture/sidetelling.md` sitt eige avsnitt "Empirisk stadfesta: header-tillit" for full detalj.
...
- **UX/Mobile Reviewer-pass (uavhengig, same dag, ekte rendra HTML+CSS): to HIGH-funn + to mindre, alle retta før merge.** Periodeval (7/30/90 dagar) bygde Oversikt-fana sitt DOM heilt på nytt, som blenka sanntidskortet tilbake til "–"-plasshaldaren heilt til neste 20-sekunders-tick -- sjølv om talet er periode-UAVHENGIG og aldri skulle endra seg av eit periodeval i utgangspunktet. Retta med ei synkron re-henting rett etter periodebyte. Kortet hadde òg ingen visuell skilnad frå dei fire periode-filtrerte korta ved sida av -- kombinert med blenkinga over kunne ein operatør lett tru talet var periode-avhengig. Retta med ein liten `border-left`-aksent (`.an-card--live`). To mindre funn òg retta: ein feila spørring vart svelgt heilt stille (umogleg å skilje "lastar enno" frå "har feila for godt") -- retta med ein `console.error`; og inga ARIA-varsling for skjermlesarar ved talendringar -- retta med `role="status" aria-live="polite"`, same mønster resten av kodebasen alt brukar for asynkrone verdioppdateringar.
- `node test.js`: 694/0 (5 nye, inkludert ein ny regresjonstest for periodeval-fiksen). `node test-workspace.js`: 210/0. `node test-api.js`: 91/0. Cache-bust: `module-sidetelling.js?v=10` (offentleg side + Web-admin).

---

## 0.107.0 — 2026-08-06

**Sidetelling utan nettlesarlagring — server-side, dagleg roterande gruppehash.** Beslutninga i `docs/compliance/legal-complexity-vs-value-2026-08-06.md` del 5 er implementert i kode: den gamle `sessionStorage`-ID-en er fjerna frå `core.js`/`module-sidetelling.js`. For sesjonsgrupperinga vert det no korkje lese frå eller skrive til cookie, browser-cache, `localStorage` eller `sessionStorage`; klienten sender ingen sesjons-ID.

- Ny Edge Function `supabase/functions/sidetelling-event` les/validerer Origin, request-IP og User-Agent frå kvar innkomande request og reknar ein 64-teikns SHA-256 av `[vibeverk-sidetelling-v1, UTC-dag, domene, IP, User-Agent]`. Berre hashen + hendingsfelta går vidare; rå IP/UA/Origin vert aldri sende til eller lagra i `analytics_events`. UTC-dagen er deterministisk og aldri persistert — ingen salttabell, nøkkelrotasjon, Vault-secret, HLL eller token-tabell. `verify_jwt=true` er eksplisitt konfigurert, og Edge-koden brukar berre innebygde Web API-ar (ingen ny CDN-avhengigheit).
- Ny deployklar migrasjon `20260806170936_server_side_daily_analytics_hash.sql` opprettar `insert_analytics_event_service()` som smal `SECURITY DEFINER VOLATILE SET search_path=public`-skriveveg med eksplisitt `EXECUTE` berre for `service_role` og ein eigen fail-closed `auth.role()`-sjekk i funksjonskroppen. Dette rettar Security Auditor sin blocker: anon-vende funksjonar skal vere `STABLE`, medan ein skrivande funksjon ikkje kan vere det. Den gamle anon-signaturen står mellombels som ein `STABLE` no-op, slik at cachelagra klientar ikkje feilar, men klient-ID/hending vert aldri lagra; ny klient kallar berre Edge.
- Ressursvern etter Security Auditor sitt HIGH-funn: ei privat `analytics_event_daily_quota`-rad per UTC-dag lagrar berre dato + globalt tal (ingen IP/hash/token) og avgrensar til 10 000 hendingar per installasjon/dag; eksisterande `session_id`-indeks avgrensar til 200 per dagsgruppe. Nådd tak er eit stille analyseavvik. Dette avgrensar lagringsvekst/enkel forgifting, men erstattar ikkje plattform-/WAF-vern mot rein volum-DoS.
- `App.getAnalyticsSessionId()` og `vw-sidetelling-session` er fjerna. Klienten bruker direkte `fetch` med anon-JWT, `credentials: "omit"` og `cache: "no-store"` i staden for Supabase SDK-en sin auth-medvitne `functions.invoke()`-veg. Runtime-testen instrumenterer jsdom sin faktiske `Storage.prototype` rundt både fyrste pageview og CTA-klikk, med ein gammal analyse-ID som sentinel, og stadfestar null lese-/skrive-/fjerningsoperasjonar, storage-fri transport og ingen session-ID/IP/UA/domene i klientpayloaden.
- Migrasjonskontrakttestar låser STABLE no-op, intern service-role-validering, eksplisitte ACL-ar, hashformat og kvotetabellen utan besøksdata. `test-api.js` importerer den ekte, reine JavaScript-handteraren (Node 20-kompatibel; Deno-entrypointet er tynt) og testar IP/site-validering, korrekt hash same dag, automatisk endring neste UTC-dag, IPv6/fallback, fråvær av rå IP/UA/site i service-kallet, stille kvotetak og generiske feil. Eksisterande paneltestar dekkjer framleis gruppering/rendering av server-returnerte `session_id`-rader.
- Dette er **ikkje unike besøkjande** og opnar ikkje ADR-0013 att: dagskoden er ein pseudonym, omtrentleg gruppe. Same IP+UA kan slå saman fleire besøk/personar same dag, og UTC-midnatt deler ei gruppe. KPI-etikettar, hjelpetekst, manual og footer omtalar difor besøksbaserte tal som daglege anslag. Datoen er heller ikkje ein hemmeleg kryptografisk salt; å hindre målretta gjenutrekning ville kravd den eksplisitt avviste, persisterte HMAC-nøkkelen.
- Personvern-standardteksten er oppdatert i både `core.js` og Console sitt versjonerte standardforslag: inga nettlesarlagring for analysegrupperinga, server-side dagskode og presis opplisting av lagra hendingar (sidevisning/henvisning/CTA, grov einingskategori og botflagg). Han seier uttrykkeleg at sjølve hendinga + dagskoden vert lagra, medan rå IP/domene/detaljert UA ikkje vert lagra i sidetellingstabellen. Påstanden er medvite avgrensa til sidetellinga (andre appfunksjonar kan bruke nettlesarlagring), og teksten skil «ingen separat analyseleverandør» frå den faktiske Supabase-databehandlaren. Dette er ei presis skildring av implementasjonen, ikkje juridisk godkjenning; Supabase sitt infrastrukturlag kan ha eigne request-loggar.
- **Uavhengig Claude Security Auditor-pass (2026-08-06, etter Codex sitt eige bygg), eitt reelt HIGH-funn retta før merge**: det globale 10 000/dag-taket kunne tømmast av eit skript (ingen nettlesar nødvendig) på nokre få sekund ved å sende mange ulike forfalska `X-Forwarded-For`-verdiar — kvar forfalska verdi gir sin eigen separate dags-hash, som kvar for seg held seg trygt under 200-taket sitt eige tak, men til saman raskt tømmer det DELTE globale budsjettet og dermed stille dreper ekte besøkjande sine hendingar resten av dagen. Retta med eit nytt, smalt minutt-nivå (`analytics_event_minute_quota`, berre minutt-bøtte + tal, 60/minutt) mellom gruppe- og dagstaket — eit raskt utført skript stoppar no etter 60 forsøk det fyrste minuttet uansett kor mange ulike hash-ar det klarer å forfalske, medan ekte trafikk for ein vanleg kundenettstad aldri i praksis råkar denne grensa. Sjølve headerrekkjefølgje-spørsmålet (kva for header som faktisk er plattform-sett) er **no empirisk stadfesta trygt (2026-08-07, sjå eiga oppføring under)** — dette reduserte skadeomfanget mens verifiseringa stod att, det var aldri ei erstatning for henne. Uavhengig Privacy Advisor-pass same runde: ingen kodenivå-blokkerarar for sjølve endringa, men peikar på eit nytt, eige Art. 6-spørsmål (rettsleg grunnlag for at Edge-funksjonen MÅ prosessere rå IP/UA for å rekne hashen, eit anna spørsmål enn ekomlov §3-15-spørsmålet denne rebuild-en løyser) som bør dokumenterast eksplisitt før reell kundeutrulling.
- Verifisert lokalt med `node test.js` (712/0), `node test-workspace.js` (210/0), `node test-api.js` (104/0) og rein `git diff --check`. Separat kode-, Security-, Privacy-/Compliance- og UX/Mobile-gjennomgang er fullført utan attverande task-spesifikk BLOCKER/HIGH; staging-/go-live-portane under står ved lag.
- Cache-bust: `core.js?v=90` på offentleg side/Web-admin/Workspace/Console, `module-sidetelling.js?v=10` på offentleg side/Web-admin, `console-core.js?v=232`. Plattformversjon: `0.107.0`.
- **Ikkje deployert/køyrd:** Edge Function-en er ikkje deploya, og ingen migrasjon er køyrd mot staging, produksjon eller kundeprosjekt. Godkjend utrullingsrekkjefølgje er Edge dormant → migrasjon (service-helper + gamal RPC no-op) → direkte staging-verifisering → cache-busta frontend. Før reell kundestart må faktiske proxy-headerar, hash/dagsskifte, kvotar og anon/RLS-/funksjonsprivilegium verifiserast. Privacy Advisor flaggar i tillegg behandlingsgrunnlag, konkret retensjon, leverandørloggar/DPA og tenantvis ny publisert personverntekst som eigne lanseringsportar; endringa er ikkje juridisk godkjenning.
...
- Ny `VIBEVERK_VENDORS`-konstant (`console/console-core.js`) — Vibeverk sine EIGNE, faste fakta om Supabase/Vercel/Resend/Plausible (kva dei gjer, land, overføringsmekanisme, DPA-status). **Hardkoda, ikkje per-kunde-data, ikkje Console-redigerbart** — dette er Vibeverk sitt eige forhold til kvar leverandør, aldri kunden sitt eige (jf. `docs/compliance/data-map-vibeverk.md` sin rolleavklaring). Domeneshop og Anthropic er MEDVITE utelatne (høvesvis sak 7 og sak 4 i beslutningsmøtet 2026-08-06) — ikkje legg dei til utan ei ny, eksplisitt avgjerd.
- Éin genuint per-kunde-redigerbar verdi: `sc.privacy.suppliers.supabaseRegion` (sak 8 i møtet). Tom verdi fell trygt tilbake til ei generell "i EU"-formulering i den genererte teksten i staden for å påstå ein stad vi ikkje har stadfesta.
- Ny `computeSupplierBlock()` genererer ei "Tredjeparter"-blokk frå desse fakta + den eksisterande `hasAnalytics`-avleiinga (Plausible kjem berre med om faktisk aktiv for kunden) — kobla inn i `computeTenantPrivacyBlocks()` som EI FLEIRE blokk, går gjennom same `mergePrivacyBlocks()`/`privacyGuardBlockedBlocks()`-maskineri som alle andre blokker frå Fase 1 (operatør-redigering vert aldri stille overskriven). `privacyModuleActive(sc, an, "suppliers")` returnerer alltid `true` — denne blokka kan aldri hybrid-vakt-blokkerast bort, sidan Supabase/Vercel/Resend strukturelt alltid er i bruk.
- Ny `renderPersonvernLeverandorer()`-fane: syner leverandørfakta read-only (namn, kva dei gjer, DPA-status-pille), pluss det eine redigerbare Supabase-region-feltet. Hentar `analytics`-nøkkelen på nytt ved kvar opning (same mønster som Dokument-fana sin "Bygg basert på gjeldande modular") for å vise om Plausible faktisk er aktiv for kunden akkurat no.
- **Ingen SQL-migrasjon, ingen ny store-nøkkel** — `suppliers` ligg inni den eksisterande `sc.privacy`-strukturen i den offentlege `superconfig`-nøkkelen (arkitekten sin grunngjeving: `supabaseRegion` er faktisk/konfig-data, ikkje upublisert utkast-prosa, difor ingen av same eksponeringsrisiko som `versions[]`/`activeVersionId` hadde i 0.100.1 — trygt å halde offentleg).
- Console-only endring — ingen `core.js`/`components.js`/`module-*.js`-endring, ingen test.js/test-workspace.js/test-api.js-påverknad (ingen av dei har jsdom-dekning for Console). Cache-bust: `console-core.js?v=227`.
- **Security Auditor-pass (uavhengig, same dag): eitt reelt MEDIUM-funn, retta før merge.** `renderPersonvernLeverandorer()` var den EINASTE Personvern-underfana som hentar noko asynkront (`analytics`-nøkkelen) FØR ho registrerer `wrap._privacyFlush` -- om ein operatør rakk å byte til ei ANNA fane, skrive noko der, og FØRST DÅ fekk dette kallet svare, ville `_privacyFlush` blitt overskrive med Leverandørar sin eigen handterar, og den andre fana sine ulagra endringar forsvunne stille (aldri kome inn i `sc.privacy`, aldri lagra). Retta med ei enkel `_privacyView`-vakt (same mønster som resten av fila alt brukar for async-sprang) -- svaret vert ignorert om brukaren har navigert vekk før det kjem attende. To LOW-funn òg retta: Resend sin DPA-status var sett til eit flatt "confirmed" der kjelda (`data-map-vibeverk.md`) sjølv berre seier "Truleg alt i kraft" -- ny mellomstatus `likely_confirmed` innført for å ikkje påstå meir enn stadfesta; og leverandør-aktiv-sjekken var hardkoda `v.id === "plausible"` på to stader -- bytt til ein delt `v.isActive(an)`-funksjon per leverandør, éin sanning i staden for to stader som kunne drive frå kvarandre.
- **UX/Mobile Reviewer-pass (uavhengig, same dag, ekte rendra HTML+CSS på fleire breidder): to HIGH-funn, retta før merge.** Den delte `.seg`-fanevekslaren (brukt av BÅDE Priser og Personvern, begge no med 5 faner) wrappa til to rader på nettbrettbreidde (768px) og smalare, og det runde `border-radius:999px`-hjørnet skar eit synleg "hakk" av bakgrunn inn i den kortare andre rada -- retta med `border-radius:22px` (framleis pilleforma på éi rad) + ein ny `@media(max-width:900px)`-regel som minskar knappepolstring for å redusere sjansen for wrapping i utgangspunktet. Supabase-region-feltet limest råtekst inn i offentleg, kundevendt personvernerklæringstekst utan noka formatrettleiing -- retta med `placeholder:"f.eks. Irland"` + eit strammare hint som eksplisitt ber om vanleg stadnamn, ikkje ein teknisk regionkode (Dashboard-navigasjonsdetaljen flytta til `help()` i staden, per copy-style-guide sitt hint/help-skilje). Eitt MEDIUM-funn retta: Leverandørar-fana brukte `getStoreKey()` (svelgjer feil stille) i staden for `getStoreKeyOrError()` for analytics-sjekken -- ein forbigåande nettverksfeil kunne vise "Plausible er IKKJE aktiv" utan varsel; retta med feil-/prøv-igjen-tilstand, same mønster som publiseringsvakta i Dokument-fana alt brukar. To attverande MEDIUM/polish-funn (pille-kontrast under WCAG AA, "✓ Lagra!" vist før stadfesta suksess) er PRE-EKSISTERANDE mønster delt med Skjematekster/Samtykker/andre stader i Console -- IKKJE retta her, treng ein eigen, repo-brei gjennomgang, ikkje eit engongsfiks i denne fana.

---

## 0.101.0 — 2026-08-06

**Fase 2 av personvern-videreutviklinga: samtykke-revisjonsspor.** Sjå `docs/compliance/legal-complexity-vs-value-2026-08-06.md` del 5 for sjølve beslutninga. Kontakt-, Tilbud- og Booking-skjemaa kan no vise valfrie, ikkje-forhandshuka samtykke-avkryssingsboksar per formål (t.d. "Send meg nyhetsbrev") — usynleg for kundar som aldri konfigurerer noko i Console sin Personvern → Samtykker-fane, sidan `consentPurposesField()` (`components.js`) rendrar tom streng når ingen aktive formål matchar skjematypen.

- `consent jsonb`-kolonne på `leads`/`bookings` (`supabase/migrations/20260806123229_add_consent_audit_trail.sql`, **ikkje enno køyrd mot nokon database** — krev eksplisitt godkjenning per driftssikringsregelen). `insert_anon_lead`/`insert_anon_booking` får ny `p_consent jsonb DEFAULT NULL`-parameter, nullbar og aldri obligatorisk.
- `App.ui.buildConsentSnapshot(container, idPrefix, formType)` (`core.js`) fryser eit augeblikksbilete ved sjølve innsendingstidspunktet: `{ formKey, privacyVersionId, privacyPublishedAt, purposes: [{ id, label, answer, at }] }` — peikar til nøyaktig kva personvernversjon som var publisert DÅ, uavhengig av seinare redigeringar.
- `consentPurposesField()` verkar for Kontakt (`components.js`), Tilbud (`module-quote.js`, fanga synkront FØR den asynkrone filopplastingskjeda for å unngå eit async-gap-tap av avkryssingssvar) og begge Booking-skjemaa (`module-booking.js`).
- Console: `privacyPublicProjection()`/`migratePrivacyPublicPart()` eksponerer no `publishedVersionId`/`publishedAt` som trygge, ufarlege peikarar i den anon-lesbare `superconfig`-nøkkelen (aldri sjølve versjonsinnhaldet — det står framleis eksklusivt i `superconfig-private`, sjå 0.100.1).
- `dbLeadToJs`/`dbBookingToJs` er kvitelistebaserte — `consent` er difor automatisk usynleg i dei vanlege Web-admin-listene/CSV/JSON-knappane ("leads-json" osv.) utan eiga ekskluderingslogikk, same mønster som `analytics_session_id` før. **Unntak, funne av Security Auditor:** den fulle "Sikkerhetskopi"-eksporten (`export_backup_tables()`) gjer ein rå radump som hoppar forbi desse kvitelistemapparane, så `consent` KJEM med der — uendra risikonivå sidan backupen alt er admin/eigar-gata og alt inneheld fullt namn/e-post/telefon/melding, men presiserer at "usynleg i eksport" berre gjeld dei vanlege listevisningane, ikkje heile-database-sikkerhetskopien.
- Ny dedikert `test.js`-blokk ("Samtykke-revisjonsspor") stadfestar formType-filtrering (kontakt/tilbud/booking), at inaktive formål (`active:false`) aldri vert vist sjølv om `forms[]` matchar, at avkryssinga aldri blokkerer innsending (uavkryssa → `answer:false`, framleis vellykka RPC), og at `buildConsentSnapshot()` returnerer `null` når ingen samtykke-seksjon finst i det heile.
- Security Auditor-pass (uavhengig, same dag): ingen blokkerande funn. To LOW-funn: ovannemnde eksport-presisering, og at `consent jsonb` ikkje har nokon storleiksgrense i migrasjonen — same (aksepterte) mønster som `attachments jsonb` sidan grunnlinja, ikkje ein ny risikoklasse denne endringa innfører, men verdt ein seinare herdingsrunde.
- UX/Mobile Reviewer-pass (uavhengig, same dag, med reelle Playwright-skjermbilete på 375px): fann at avkryssingsboksane hadde INGEN eigen CSS i det heile — 0px avstand til den obligatoriske vilkårsboksa på Tilbud spesielt, og ingen visuell skilnad mellom obligatorisk/valfritt nokon stad. Retta: ny delt `.consent-purposes`/`.consent-purpose-row`-styling (radlayout, større avkryssingsboks, hengande innrykk for fleirlinja etiketttekst) lagt til i `index.html`, `admin/index.html` OG `module-quote.js` sin eigen injiserte stilark (Tilbud sitt steg-2-skjema er eit bart `<form>` utan den delte grid-gap-en dei to andre skjemaa har). Kvar boks får no òg ei synleg "(valgfritt)"-merking i `consentPurposesField()` (`components.js`), slik at "dette er IKKJE obligatorisk, ulikt boksen over" faktisk kan sjåast, ikkje berre sluttast frå fråvær av feilmelding.
...
- `sc.privacy` veks frå `{heading, text}` til ein strukturert form: `activeVersionId`, `versions[]` (versjonerte, med `status` draft/published, uforanderleg når publisert), `forms{}` (per-skjematype-tekster: formål, behandlingsgrunnlag, lagringstid, mottakarar, korttekst), `consentPurposes[]` (samtykkeformål-DEFINISJONAR, ingen logging enno). `heading`/`text` står FRAMLEIS på toppnivå, urørt, som ein flat projeksjon av den publiserte versjonen — nettsida (`core.js`) treng ingen kodeendring.
- Ny Console-seksjon med fire faner: **Dokument** (avsnitt merka `Modul: X`/`Manuelt`, "Bygg basert på gjeldande modular" slår saman friskt forslag med manuelle/redigerte avsnitt utan å overskrive, hybrid-vakt nektar publisering om eit avsnitt knytt til ein AKTIV modul er ekskludert), **Skjematekster** (kontakt/tilbud/booking/nyhetsbrev, med tomt `Behandlingsgrunnlag`-felt — ikkje asserta, avgjerast av operatør/jurist), **Samtykker** (formål-definisjonar, alltid ikkje-forhandsavhuka), **Historikk** (tidlegare versjonar, "Bruk som utgangspunkt for nytt utkast").
- **Medvite fjerna frå det auto-genererte forslaget**: den gamle "Ved å sende inn dette skjemaet samtykker du..."-utsegna. Beslutningsmøtet 2026-08-06 fann (Datatilsynet-grunngjeve) at samtykke truleg IKKJE er rett behandlingsgrunnlag for eit vanleg skjema — forslaget skal ikkje lenger påstå eit grunnlag som ikkje er stadfesta.
- **Retta ein reell drift** mellom `core.js` og `console-core.js` sine to forslagsgeneratorar: Console-versjonen mangla sidetelling-medvit cookieText-grein (berre Plausible/ingen, ikkje Plausible/sidetelling/ingen).
- UX/Mobile Reviewer-pass same dag fann og retta ein reell BLOKKERANDE feil før merge: skrivne-men-ikkje-lagra endringar i EIN fane forsvann stille ved intern fanebyte (Dokument→Skjematekster osv.), sidan `captureFieldEdits()`/tilsvarande berre vart kalla frå spesifikke knappar, aldri frå sjølve fanebyte-handteraren. Fiksa med eit felles `wrap._privacyFlush`-hekt kvar fane registrerer, kalla av dispatcheren FØR fanebyte. To relaterte HIGH-funn retta same runde: eit tilsvarande asynkront hòl i "Bygg basert på gjeldande modular"/"Publiser" (edits gjort MEDAN eit nettverkskall var undervegs kunne forsvinne), og at ein feila `getStoreKey()`-spørring stille vart tolka som "ingen analyseverktøy aktivt" i publiserings-vakta — retta med ein ny `getStoreKeyOrError()` som avbryt publiseringa med ei tydeleg feilmelding i staden. Mindre funn retta same runde: Tier B-forbetra publiser-stadfestingstekst, fjerna eit unødvendig `confirm()` på ei fullt reversibel Tier A-handling (gjenopprett gammal versjon som nytt utkast), lagt til hint på "Mottakere"/"Lagringstid"-felt, hint om at "Aktiv"-brytaren på samtykkeformål ikkje har synleg effekt enno, og attbruk av Console sin eksisterande `.kd-pill`-statusmerke-komponent i staden for tilfeldig inline-farge.

Stadfesta med eit frittståande Node-skript (migrering, sammenslåing av modul-/manuelle avsnitt, hybrid-vakt, versjonskloning). Alle tre testsuitane (`test.js`/`test-workspace.js`/`test-api.js`) framleis 0 FEIL (Console har ingen jsdom-dekning). Fase 2–5 (samtykke-revisjonsspor, leverandørregister, godkjenningsflyt, endringsvarsling) er eksplisitt IKKJE ein del av denne oppføringa. Cache-bust: `console-core.js?v=224`.
