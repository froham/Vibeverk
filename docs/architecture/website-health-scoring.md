...
- **"Cookies" consent** — dropped; both supported analytics choices (Plausible and the mutually exclusive internal sidetelling) are designed without analytics cookies. The internal option also avoids browser storage for analytics grouping. Revisit only if a cookie-/storage-setting analytics integration is ever added; this scoring omission is not a legal-compliance conclusion.
...
**Follow-up fix, same day (0.77.1)**: the initial Console integration missed that `renderNettsidehelseSection()`'s HTML depends on CSS classes (`.an-heading`/`.an-cards`/`.an-card*`, `.prose`, `.lead-details`) that only existed in `admin/index.html`, never in `console/index.html` — so the section rendered without borders/card backgrounds, breaking Console's otherwise-consistent boxed-fieldset look (caught by the user from a live screenshot). Fixed by adding the missing rules to `console/index.html` and changing the `#cs-nettsidehelse` wrapper from a plain `<div>` to a `<fieldset class="admin-group">` (no `<legend>`, since the section already renders its own `<h4>` heading) — same box treatment as every other Console section. Verified via a Playwright screenshot of a static reconstruction (Console's real CSS + real `renderNettsidehelseSection()` output) at both mobile and desktop widths, not a live logged-in pass.
