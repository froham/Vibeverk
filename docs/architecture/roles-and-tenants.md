...
- `supabase/hotfix_tasks_description_editable_2026-07-03.sql` (step 5 above — extends `restrict_assignee_task_columns()` to also permit `description` changes, not just `status`, for the assigned-by-others case): folded into `supabase/migration.sql`, **not yet run against production** — awaiting explicit user approval.
