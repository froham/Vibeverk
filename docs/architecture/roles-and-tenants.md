...
**Tasks self-create/self-edit caveat (corrected 2026-07-02, then partly reverted 2026-07-03):** the task spec this matrix was originally built from said member should not be able to create tasks at all. The user corrected this progressively: (1) member should be able to create a task for themselves, just not assign one to someone else; (2) member should also be able to *fully edit* (title/description/due date, not just status) a task they created themselves; (3) a task **assigned to member by someone else** should be *fully* read-only — not even status-changeable. **Step (3) was itself reverted the next day (2026-07-03)**: the user clarified that changing status on a task assigned by someone else is expected, everyday behaviour and must remain possible — it was never meant to be blocked. The final rule, implemented in `intranet/module-tasks.js` and enforced server-side:
...
- A task **assigned to the member by someone else** (`created_by` is not them, `assigned_to` is them): status-only — the status `<select>` on the row remains editable, but no other field (title/description/due date/assignee) can be changed. Row click does not open the edit modal for this case (`openTaskModal()` no-ops).
