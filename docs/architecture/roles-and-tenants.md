...
- `supabase/hotfix_tasks_readonly_for_assigned_2026-07-02.sql` (step 3 above — narrows `tasks_assignee` to `created_by = auth.uid()` only, simplifies the trigger accordingly): folded into `supabase/migration.sql`, **run against production and confirmed 2026-07-02** via `npx supabase db query --linked`, verified against `pg_policies`.
