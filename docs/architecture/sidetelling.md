...
- **Geolokasjon**: **Revurdert 2026-08-10 (Arkitekt-spike, brukar ba om ei ny vurdering av ein "mellomveg")** — den opphavlege grunngjevinga over ("krev anten eit eksternt API-kall eller lagring av IP-adresse") synte seg for absolutt. Ein lokal IP-range→landkode-oppslagstabell (t.d. bygd frå DB-IP Lite, CC BY 4.0, berre attribusjonskrav, ingen konto/lisensnøkkel/EULA slik MaxMind GeoLite2 krev) kan pakkast som eit lite (~2 MB) statisk innebygd oppslag i sjølve `sidetelling-event`-funksjonen — ingen eksterne API-kall ved request-tid (prinsipp 2 uendra), og rå IP vert framleis aldri lagra (same "avled ein smal kategori, kast rådata"-mønster som `device_type`/`is_bot` alt bruker). Teknisk **vesentleg billigare** enn den avviste unik-besøkjande-HLL-en (ADR-0013) -- ingen kryptografi, nøkkelrotasjon, `pg_cron` eller Vault-secret, berre éin ny nullbar kolonne + éin liten binærsøk-funksjon.

  **Likevel ikkje bygd** -- ikkje lenger fordi det bryt eit prinsipp, men fordi verdien er tynn for Vibeverk sine faktiske kundar (små norske SMB-ar, nesten utelukkande innanlands trafikk): eit "besøkjande per land"-kart for denne målgruppa ville i praksis vist "~97 % Noreg, 3 % støy" -- ikkje eit tal nokon kunde faktisk handlar på, ulikt referrer/CTA/eining-tala som alt finst. Same konklusjonsmønster som ADR-0013 (avvist, ikkje berre utsett), men av ein annan grunn (tynn verdi, ikkje uforholdsmessig kompleksitet) og med ei lågare terskel for å ta opp att seinare -- om ein konkret kunde med reell internasjonal trafikk nokon gong etterspør dette, er oppskrifta over klar til å hentast fram.
