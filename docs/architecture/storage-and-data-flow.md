...
Migration for existing production data: `supabase/hotfix_bookings_data_migration_2026-07-03.sql`, idempotent, does not delete the old `store` row. As of this writing, neither the table DDL nor the data migration has been run against production.
...
|---|---|---|
| `store` | Key/value config, superconfig, `booking-assets`, `crm-settings` — CRM customers/leads/bookings all moved out to real tables 2026-07-03 (leads/bookings' SQL not yet run, see above) | See "Supabase store table" above — as designed (not yet fully live, see caveat there): anon `SELECT` on all rows (known finding), authenticated `SELECT` on all rows, writes gated by key |
