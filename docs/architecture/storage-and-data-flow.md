...
RLS on `store` as defined in `supabase/migration.sql` (verified 2026-07-02): anon has `SELECT` on all rows (`store_anon_read`, `USING (true)`) — this is a known, separately-tracked finding (`docs/project/CURRENT_STATE.md` "Still open" — anon can read `booking-bookings`, the one remaining private-data key still stored in this table; **CRM data and leads were moved out 2026-07-03, see below**). Authenticated users get `SELECT` on all rows via `store_read_authenticated`. Writes to `store` are split across three command-specific policies (`store_insert_auth`, `store_update_auth`, `store_delete_auth` — previously one combined `FOR ALL` policy called `store_auth`, replaced 2026-07-02 for the reason below). INSERT/UPDATE require `is_admin_or_owner()` for the `superconfig` and `wsp-orgdrift` keys, `can_edit_content()` (admin/editor) for most other keys. `store_read_authenticated` and the `wsp-orgdrift` write carve-out were added 2026-07-02 (`supabase/hotfix_role_enforcement_2026-07-02.sql`) — **run against production and confirmed by the user 2026-07-02** (see `docs/security/security-baseline.md`).

**CRM data and leads moved out of `store` 2026-07-03 (fixes the CRITICAL anon-SELECT finding for these keys):** `crm-customers`/`crm-bedrifter`/`crm-comms` (2026-07-03, first pass) and `leads` (2026-07-03, second pass — Kontakt AND Tilbud submissions, previously distinguished only by message-text sniffing, not a real field) are no longer `store` keys. The `CASE` branches referencing them in `store_insert_auth`/`store_update_auth` are now dead (harmless, kept until the old `store` rows are verified-and-deleted in a follow-up cleanup). This data now lives in real tables — see "CRM tables" and "leads table" below. `crm-settings` (templates/snippets/signatures — no PII) remains in `store`, unaffected. `booking-bookings` is planned as the final step of the same effort and is **still** in `store`, still subject to the anon-SELECT finding, as of this writing.
...
Migration for existing production data: `supabase/hotfix_leads_data_migration_2026-07-03.sql`, idempotent, sets `kind` for existing rows via the old text-sniffing rule, does not delete the old `store` row. As of this writing, neither the table DDL nor the data migration has been run against production.
