...
| [tracing.md](tracing.md) | W3C Trace Context (`traceparent`) request tracing: generation/forwarding chain across `middleware.js`, `api/*.js` and the client-side `core.js`/`console-core.js`/`workspace-core.js`, why it exists, and what it deliberately is not (no persistence, no per-query resolution, no log viewer) |
