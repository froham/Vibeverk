...
1. **Sidetelling — sessionStorage-lagring**: Avgjerda var fyrst å pause for reelle kundar (behald for Vibeverk sjølv/Sunnvask-demo) og byggje om mekanismen til ein server-side, dagleg-roterande hash av IP+User-Agent (same prinsipp som Plausible sin eigen offentleg dokumenterte metode, stadfesta via `plausible.io/data-policy` 2026-08-06 — "We do not use cookies, browser cache or local storage", `hash(daily_salt + domain + ip + user_agent)`). **Implementert i kode 2026-08-06 (v0.106.0):** klienten har ikkje lenger analyse-ID og les/skriv ingen cookie, `localStorage` eller `sessionStorage` for grupperinga; Edge Function-en `sidetelling-event` reknar ein SHA-256 av UTC-dag + Origin/domene + request-IP + User-Agent ved kvart kall og sender berre hashen/hendinga til ein service-only skrive-RPC. Dagsverdien er deterministisk og ikkje persistert — ingen salttabell, nøkkelrotasjon, HLL eller token. Ein separat global kvotetabell inneheld berre UTC-dato + hendingstal (ingen besøksidentitet) som ressursvern. Edge-funksjonen er ikkje deploya og migrasjonen er ikkje køyrd mot staging/produksjon utan eksplisitt godkjenning. Hashen er pseudonym, ikkje irreversibelt anonym, og den tekniske endringa er ikkje i seg sjølv ei juridisk samsvarsgaranti. Viktig: løysinga løyser framleis IKKJE sak 2 (sjå punkt 2 under, dei er sjølvstendige spørsmål).
