...
>
> **Oppdatert 2026-08-13** etter kodegjennomgang av Arctic/AI Lab: lagt til den lokale operatør-/modell-auditten, den tidsavgrensa snapshotbehandlinga og den særskilt gatede Anthropic-flyten. Den eldre 2026-08-06-konklusjonen om at ingen automatisk retensjon fanst er framleis rett for kundedata-kategoriane som vart undersøkte då, men gjeld ikkje Arctic si nye lokale auditfil: denne har no kodefesta sletting etter 30 dagar ved prosessoppstart og skriving.
...
| Felt | Detalj |
|---|---|
| Data samla inn | Sidesti (hash-rute), `referrer` (kun vertsnamn), CTA-type, `device_type` (kategorisk) og `is_bot`. For kvar hending bruker Edge Function-en mellombels Origin/domene, request-IP og User-Agent til å rekne ein dagsavgrensa SHA-256-grupperingskode. Berre koden vert sendt vidare og lagra i `session_id`; rå IP/UA/Origin vert ikkje lagra i `analytics_events`. Ingen analyse-ID, cookie, `localStorage` eller `sessionStorage` i nettlesaren for grupperinga. Konverteringskobling til `leads`/`bookings` er fjerna frå skrivesida. |
| Kor lagra | Hending + pseudonym dagskode i `analytics_events`-tabellen i kunden sin eigen data-plane. Ei privat ressursverntabell lagrar berre UTC-dato + globalt hendingstal, ingen hash eller besøksdata. Supabase sitt Edge-/infrastrukturlag kan ha eigne request-loggar; dette kartpunktet skildrar Vibeverk-koden/tabellane, ikkje ein påstand om alle leverandørloggar. |
| Rettsleg grunnlag | Den konkrete §3-15-risikoen frå lagring/lesing i brukarutstyret er fjerna strukturelt i v0.106.0: sidetellinga bruker inga nettlesarlagring. Dagskoden er likevel pseudonym persondata, ikkje irreversibelt anonym, så vanlege GDPR-plikter (formål, informasjon, behandlingsgrunnlag, retensjon m.m.) må framleis vurderast. **Nytt, separat spørsmål frå v0.106.0 (Privacy Advisor-funn, 2026-08-06), ikkje berre ei vidareføring av §3-15-spørsmålet over**: Edge-funksjonen må no *prosessere* (ikkje lagre, men lese og rekne på) den rå IP-adressa og User-Agent-strengen på KVAR einaste hending for å rekne hashen — den gamle mekanismen brukte aldri IP-adressa i det heile. IP-adresser er personopplysningar (jf. Breyer-saka), og prosessering krev eige rettsleg grunnlag etter GDPR art. 6, uavhengig av at §3-15-spørsmålet no er løyst. Sannsynleg grunnlag: legitim interesse (art. 6(1)(f)), men det krev ei dokumentert interesseavveging (formål/nødvendigheit/personvernkonsekvens/forholdsmessigheit) som **ikkje er gjort enno**. Ingen KI-gjennomgang er juridisk godkjenning. Konverteringskoblinga er eit separat spørsmål og står framleis fjerna. |
...
### Kundeanalyse i Console — kodeklar 2026-08-10 (utvida same dag med reell sletting), migrasjon kjørt og verifisert mot `vibeverk-control` same dag, men ikkje funksjonell i produksjon (service-role-hemmelegheit ikkje sett)
...
| Hvor lagret | `vibeverk-control`-tabeller med prefiks `customer_analysis_`, ikke i kundens data-plane. Hele HTML-sider lagres ikke. Migrasjonen er kjørt og verifisert (tabeller, RLS, policyer, grants) 2026-08-10 — men funksjonen er ikke funksjonell før `VIBEVERK_CONTROL_SERVICE_ROLE_KEY` er satt i Vercel-miljøet. |
...
### Arctic / AI Lab — intern operatørflate, releasekandidat 2026-08-13 (ikkje deploya)

| Felt | Detalj |
|---|---|
| Formål | Tenant-uavhengig, superadmin-avgrensa driftsstatus og lokalt modellverkstad for naturleg Gemma-samtale, eksplisitt tekst-/kildeanalyse og strukturerte forslag til Læringsmodulen. Ingen automatisk publisering, fri filtilgang eller Claude-/Codex-arbeidsøktmotor. |
| Data behandla lokalt | Samtalemeldingar, opptil 20 000 teikn innlimt tekst, valde allowlista repofiler, operatøren si læringsinstruksjon, modellresultat og valfri vurdering/kommentar. Generelle kontekstar og læringssnapshot ligg tidsavgrensa i Node-prosessminne; nettlesaren held inntil ti økter og arbeidsflata i JS-minne fram til reload eller «Tøm arbeidsflaten». Ingen av desse innhaldsdataa vert skrivne til Supabase, `localStorage` eller database av AI Lab-koden. |
| Lokal audit | Registrerte kommandokall og modellkøyringar skriv metadata-only NDJSON under gitignorert `ARCTIC_RUNTIME_DIR`: operatør-ID, action/resultat/feilklasse/tid og, for modellkall, provider-/modell-ID, lokal/ekstern behandling og kjelde-ID-ar. Innhaldshash, instruksjon, kjeldeinnhald, prompt, modelloutput, kommentar, token og secrets vert ikkje logga. Operatør-ID og bruksmetadata er likevel personopplysningar om Vibeverk sin operatør. |
| Retensjon/sletting | Generell kontekst vert eksplisitt disponert etter modellkallet og har i tillegg aktiv TTL; læringssnapshot vert disponert ved endring/tømming og har TTL/kapasitetsutkastelse som sikkerheitsnett. Primærbuffer/referansar vert nullstilte/tømde når snapshotet kan disponerast, men mellombelse JS-strengar, in-flight kall og fysisk GC-overskriving kan ikkje garanterast. Auditfiler vert roterte ved UTC-døgnskifte eller 1 MiB, har 10 MiB samla fail-closed tak og filer eldre enn 30 dagar vert sletta ved prosessoppstart og før skriving (ingen bakgrunnsjobb når prosessen er inaktiv). Døgnrotasjonen hindrar at ny aktivitet kontinuerleg forlengjer levetida til eldre postar. «Tøm arbeidsflaten» påverkar ikkje eksporterte filer eller providerdata. |
| Tilgang | UI-et og kvart API-kall krev ein control-plane-operatør med `status='active'` og `role='superadmin'`; lokale POST-kall krev i tillegg same origin, CSRF og prosessunik handlingstoken. Produksjonsseamen kontaktar ingen privat maskin og returnerer berre `gateway_not_configured`. |
| Ekstern behandling | Haiku/review kan sende eksplisitt valde kjeldeutdrag og instruksjon til Anthropic; review sender òg Gemma-utkastet. Kallet krev API-nøkkel, `AI_LAB_ANTHROPIC_PROCESSING_APPROVED=true`, kjeldebytes som matcher godkjend SHA-256, best-effort tripwire utan kjende secrets og ny UI-stadfesting per kall. Kontrollane er ikkje anonymisering eller dokumentasjon på lovleg behandlingsgrunn. |
| Behandlingsgrunnlag og informasjon | **[ÅPENT — organisatorisk/juridisk lanseringsport]** for operatøraudit og eventuell ekstern behandling: formål/nødvendigheit, informasjon til operatørar, forholdsmessig 30-dagars auditretensjon, tillatne datakategoriar og Anthropic-avtale/DPA/overføring må stadfestast av ansvarleg person. Sjå `docs/architecture/arctic.md` og `docs/architecture/ai-lab.md`. |
...
| Anthropic PBC | AI-generering for Oversikt/Smart årshjul (Workspace), Kundeanalyse og den lokale AI Lab-flyten | Workspace: brukarskriven situasjons-/årshjulstekst. Kundeanalyse: avgrensa offentlege tekstutdrag/funn. AI Lab: eksplisitt valde allowlista kjeldeutdrag, operatørinstruksjon og ved review eit Gemma-utkast; kan innehalde persondata dersom kjeldeklassifisering eller operatørinput er feil | **JA for Workspace/Kundeanalyse. AI Lab-adapteren finst i releasekandidaten, men ekstern bruk er deaktivert utan både API-nøkkel og `AI_LAB_ANTHROPIC_PROCESSING_APPROVED=true`; Arctic-runden er ikkje deploya eller produksjonskonfigurert.** | **[ÅPENT — Anthropic sin faktiske DPA, underdatabehandlarliste, region/retensjon og overføringsgrunnlag må undersøkast før AI Lab-porten kan opnast]** | **[ÅPENT]** |
...
- **QR-modul** (`module-qrcode.js`, `api/qr-redirect.js`, 0.151.0, lagt til 2026-08-19): `middleware.js` rewriter `/qr/<code>` til `/api/qr-redirect`, som slår opp tenant frå `Host`-headeren og kallar den trange `get_qr_redirect_target(code, tenant_id)`-RPC-en i **kundens eige** data-plane-prosjekt (ikkje `vibeverk-control`). RPC-en returnerer berre mål-URL-en for éin aktiv, tenant-scoped kode og gjer eit 302-redirect. Ingen personopplysning om den skannande besøkjande vert samla inn eller logga av denne mekanismen — han løyser berre KVA mål-URL koden peikar på, same avgrensa rolle som ruting-mekanismen elles i dette punktet.
...
| Vibeverk-operatør (Console) | Aktive operatørar: superconfig, produktmodus og funksjonsflagg for den enkelte tenanten via `broker`-funksjonen. Berre aktive `superadmin`-operatørar: Arctic/AI Lab, med lokal metadata-audit som skildra over. Ingen direkte tilgang til kunden sin `store`-tabell utanom broker-vegen (per CLAUDE.md sin regel). |
...
- [x] **Teknisk lukka i kode 2026-08-06 (v0.106.0), live-verifisering står att**: sidetelling si `sessionStorage`-lagring er fjerna frå ny klient og erstatta med Edge-basert dagskode. Edge-funksjonen er ikkje deploya og migrasjonen er ikkje køyrd enno; request-headerflyt, kvotar og tilgangar må verifiserast på staging før reell kundestart. Konverteringskoblinga er òg fjerna. Dette er ikkje ei juridisk samsvarsgaranti for den attverande pseudonyme behandlinga — sjå `docs/architecture/sidetelling.md` og beslutningsdokumentet del 5.
- [ ] **Oppdatert 2026-08-13**: Anthropic sin DPA/underdatabehandlarliste/overføringsgrunnlag, region og faktiske retensjon for Oversikt/Smart årshjul, Kundeanalyse og den gatede AI Lab-flyten — ikkje stadfesta i repoet.
- [ ] **Oppdatert 2026-08-13**: automatisk retensjon/sletting finst framleis ikkje for kundedata-kategoriane som vart kartlagde 2026-08-06. Arctic sitt nye lokale metadata-audit er eit avgrensa unntak med 30-dagars filsletting ved prosessoppstart/skriving; om perioden er forholdsmessig og korleis operatørane skal informerast er ope.
