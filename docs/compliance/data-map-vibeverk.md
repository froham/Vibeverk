...
Stadfesta reell og aktiv: `core.js` linje 742 og `workspace/workspace-core.js` linje 103 byggjer dynamisk ein `<link>` til `fonts.googleapis.com` ut frå `config.js.fonts`, injisert i `<head>` ved sideinnlasting. `index.html`/`admin/index.html`/`workspace/index.html`/`console/index.html` har alle CSP-oppføringar for `fonts.googleapis.com`/`fonts.gstatic.com`. Sender ein nettlesarførespurnad til Google sin CDN for kvar sidevising — IP-adresse kan bli logga av Google, same konklusjon som malen alt hadde, no stadfesta i kode for FIRE overflater (nettside, web-admin, Workspace, Console), ikkje berre den offentlege sida.
...
| Resend | Transaksjonell e-post: chat/kontakt/tilbud-svar (`send-reply`) OG Supabase Auth SMTP (Console sin "Set opp e-post"-steg, per onboarding-runbooken) | Mottakars e-postadresse, namn, emne, meldingstekst, ev. vedlegg (`send-reply`); og invitasjons-/passord-reset-e-postar (Auth SMTP) | JA — to ulike bruksområde, stadfesta i kode/runbook | **[ÅPENT — Resend er eit amerikansk selskap; overføringsgrunnlag ikkje stadfesta]** | **[ÅPENT]** |
| Google LLC | Google Fonts-levering | Besøkjande sin IP (hendeleg, ved fontførespurnad) | JA — alle fire overflater | USA | **[ÅPENT, sannsynlegvis Google sine standardvilkår]** |
| Plausible Analytics | Web-analyse | Sidevisingar, ikkje-cookiebasert | **[ÅPENT for DENNE tenanten — sjå punkt 3a, ikkje konfigurert i repoets `config.js`]** | Sannsynlegvis EU-basert (ikkje uavhengig stadfesta av dette passet) | **[ÅPENT]** |
