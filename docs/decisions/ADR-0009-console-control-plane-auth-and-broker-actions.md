...
No backend/Supabase changes needed for the `superconfig` fix itself (`store_anon_read` already permits the new read pattern). The `analytics` fix required a `broker` redeploy (new allowed key) — **deployed 2026-07-09** (`npx supabase functions deploy broker --project-ref jxoglthrnshabqmdmnui --workdir supabase-control`), verified live via a negative-path check (no `Authorization` header → 401), not just a clean deploy message.
