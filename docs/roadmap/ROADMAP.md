...
- ~~Rett Console sin sesjonssjekk-ved-navigering-svakheit~~ — **lukka 2026-07-07**, sjå Current focus over.
- ~~SQL-migrasjonskonvertering (Fase 3 av SaaS-skaleringsplanen)~~ — **fullført 2026-07-07**: `supabase/migrations/` er no den ekte, CLI-deployerbare kjelda (baseline `20260707000001`, produksjon baseline-merkt utan re-køyring, stadfesta via `supabase migration list --linked` + `db push --dry-run`). `migration.sql` er merkt overtatt/frose. Sjå `docs/project/CHANGELOG.md` 0.18.0 og `CLAUDE.md` sine oppdaterte Supabase-reglar.
- ~~Asynkron config-bootstrap-plumbing (Fase 4 av SaaS-skaleringsplanen, ADR-0007 sin eigen "Fase 1")~~ — **fullført 2026-07-07**: `App.ready(fn)`-gate lagt til i `core.js`, alle 7 rot- + 8 intranett-modulfiler + `intranet-core.js` + `console-core.js` + det bespoke `module-chat.js` går no via gaten. Reint plumbing i denne omgangen — `config.js` er framleis ein synkron `<script>`-tag, ingen HTML-endringar. Sjå `docs/project/CHANGELOG.md` 0.19.0 for full detalj, inkludert eit funne-og-fiksa `_sb`-snapshot-gap i `module-users.js` og eit dokumentert, ikkje-fiksa tilsvarande gap i `core.js` sin eigen `App.supabase`-konstruksjon (krev å omstrukturere heile return-objektet, utanfor denne fasen sitt omfang).
- ~~Namnebyte intranet → Workspace (Fase 5 av SaaS-skaleringsplanen)~~ — **fullført 2026-07-07 (Tier 1)**: `intranet/` → `workspace/`, `intranet-core.js` → `workspace-core.js`, `test-intranet.js` → `test-workspace.js`, alle funksjonelle lenker/URL-stiar oppdaterte, redirect-snubbe att-lete på gamle `/intranet/`. **Tier 2 (`window.Intranet`, `#intranet`-DOM-IDar, `intranettFeatures`-nøkkelen) er med vilje IKKJE gjort no** — separat, seinare, ikkje-hasta steg, sjå `docs/project/CHANGELOG.md` 0.20.0 for grunngjeving. **Attståande før deploy**: `hub/tenants.js` sin `workspaceUrl`-verdi må oppdaterast i same commit som den faktiske deployen (ikkje før), og sjølve `git push` krev eksplisitt godkjenning. **Neste steg i SaaS-skaleringsplanen: Fase 6, hostname-basert tenant-oppløysing.**
