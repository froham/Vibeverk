...
5. **Automatisert livetest-agent (smoke-test) for heile plattforma.** Stadfesta 2026-07-13, kjem etter dagens gjenståande 0.32.0-arbeid (prod-godkjenning, Privacy/UX-gjennomgangar, brukar sine manuelle testar). Ein gjenbrukbar, skriptbasert (Playwright) testpakke som faktisk driv ein ekte nettlesar gjennom kjerneflytene — innlogging (web-admin/Workspace/Console), CRM, booking, tilbod/kontaktskjema, chat-widget, KB, kunngjeringar, oppgåver, brukaradmin, og Console sin onboarding-sjekkliste — i staden for berre jsdom-logikktestane (`test.js`/`test-workspace.js`) eller éin-gongs manuell Playwright-verifisering (som Fase 9 sin kanarikunde fekk). Skal køyrast mot `vibeverk-staging`/ein dedikert testtenant, ALDRI direkte og ukritisk mot produksjon (skriv/slettar ekte testdata). Tenkt både manuelt utløyst og som ein tidsstyrt rutine (cron/`schedule`) som varslar ved feil — særleg verdifullt rett etter kvar ny kunde-onboarding, som ei repeterbar "er alt likt originalen"-sjekk i staden for tillit til éin manuell gjennomgang. Bygg gradvis: start med dei flytene som nyleg var buggy (brukarsletting, backup/restore, dashboard-snarvegar), utvid derifrå. Eige arbeidsstykke — bør gå via Arkitekt- eller QA-agenten for testdekning-design før bygging.
