...
(function () {
  ...
  function renderAdminChat(container) {
    ...
    function buildUI() {
        ...
        '<div class="vwca">' +
          ...
          '<div class="vwca-list">' +
            '<div class="vwca-list-head">' +
              ...
              '<button id="vwca-new-test" title="Opprett testsamtale" style="background:none;border:1px solid var(--color-border);border-radius:6px;cursor:pointer;padding:.15rem .5rem;font-size:.75rem;color:var(--color-muted)">+ Test</button>' +
            '</div>' +
            (convs.length
      ...
      /* Testknapp — opprett fiktiv innkommande samtale */
      var testBtn = container.querySelector("#vwca-new-test");
      if (testBtn) {
        testBtn.addEventListener("click", function () {
          var names   = ["Kari Nordmann","Ola Hansen","Per Olsen","Anna Berg","Lars Dahl","Mette Vik"];
          var msgs    = [
            "Hei! Eg lurer på prisane dykkar.",
            "Kan eg bestille time?",
            "Hei, har de ledig kapasitet neste veke?",
            "Eg fekk ein feil på nettsida — kan nokon hjelpe?",
            "Er de opne i helgane?",
            "Hei! Korleis kjem eg i gang?"
          ];
